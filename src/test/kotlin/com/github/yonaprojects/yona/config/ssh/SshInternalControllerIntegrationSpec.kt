...
/**
 * yona-wiki P3-03 Step4 — `yona internal ssh-auth`/`ssh-shell`(yona-cli)이 호출할 내부 엔드포인트
 * 자체의 동작을 검증한다. 실제 시스템 sshd 연동은 호스트 시스템을 건드릴 수 없어(작업 지시) 이
 * 세션에서 수행할 수 없으므로, "sshd가 이 API를 이렇게 호출하면 이렇게 응답한다"는 계약을 실제
 * HTTP 요청(JDK HttpClient)으로 검증하는 것으로 대체한다.
 */
...
class SshInternalControllerIntegrationSpec @Autowired constructor(
    ...
    init {
        describe("SshInternalController") {
            ...
            it("authorize는 principal이 스코프 내 저장소에 대한 git-upload-pack을 요청하면 allowed=true를 반환해야 한다") {
                val project = projectRepository.save(Project(owner = "gildong", name = "internal-repo"))
                val issued = deployKeyService.create(project, "internal 테스트 키", USER_PUBLIC_KEY, readOnly = true)
                val principal = "deploykey:${issued.deployKey.id}"

                val response = postJson(
                    "/internal/ssh/authorize",
                    """{"principal":"$principal","command":"git-upload-pack '/gildong/internal-repo.git'"}""",
                    secret = secretProvider.secret
                )

                response.statusCode() shouldBe 200
                response.body() shouldContain "\"allowed\":true"
                response.body() shouldContain "gildong/internal-repo.git"
            }

            it("authorize는 알 수 없는 principal에는 allowed=false를 반환해야 한다") {
                val response = postJson(
                    "/internal/ssh/authorize",
                    """{"principal":"deploykey:999999","command":"git-upload-pack '/gildong/no-such-repo.git'"}""",
                    secret = secretProvider.secret
                )

                response.statusCode() shouldBe 200
                response.body() shouldContain "\"allowed\":false"
            }
