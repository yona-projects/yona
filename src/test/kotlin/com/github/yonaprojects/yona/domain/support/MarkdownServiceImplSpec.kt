...
class MarkdownServiceImplSpec : DescribeSpec({
    ...
    describe("MarkdownServiceImpl 새니타이저 XSS 방지 검증 (allowlist 기반)") {
        ...
        // commonmark 확장 목록에 task-list-items가 빠지면 "- [ ] task"가 <input type="checkbox">가
        // 아니라 리터럴 텍스트로 남아, common/yona.Tasklist.js가 체크박스를 찾지 못해 토글 기능이
        // 죽는다(Playwright로 재현). 새니타이저 allowlist는 이미 input을 허용하므로 확장 누락이 원인이었다.
        ...
        // commonmark-ext-task-list-items는 항상 disabled=""를 붙인다. legacy(marked.js)는 붙이지
        // 않고 common/yona.Tasklist.js가 수정 권한 없는 뷰어에게만 사후에 disabled를 붙이는데,
        // 그 JS는 disabled를 벗겨내는 로직이 없어 렌더러가 항상 붙이면 아무도 클릭할 수 없다
        // (Playwright로 확인). legacy처럼 렌더러 출력에서 disabled를 제거해 권한 판단은 JS에 맡긴다.
