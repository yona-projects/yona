...
// legacy common/yona.CommentForm.js(빈 값 제출 방지, Ctrl+Shift+Enter 단축 제출, beforeunload
// 경고, localStorage 임시저장)의 기능적 동치를 #comment-form에 이식했다. 원본 파일을 그대로
// <script src>로 로드하지는 않는다 — onSubmitCommentForm이 event.preventDefault() 후 300ms 뒤
// 네이티브 form.submit()을 다시 스케줄링하는데, #comment-form은 이미 AJAX 제출 핸들러
// ($(document).on('submit', ...))로 전환돼 있어 공존시키면 이중 제출 버그가 난다 — 그래서 그
// AJAX 핸들러 쪽에 4가지 기능을 직접 재구현했다. yona.CommentForm.js 파일 자체는 삭제하지 않고
// 계속 미참조 상태로 남겨둔다.
@Transactional
