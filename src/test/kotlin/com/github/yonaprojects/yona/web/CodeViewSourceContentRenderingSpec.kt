...
// 코드 브라우저에서 바이너리도 마크다운도 아닌 일반 텍스트/소스 코드 파일을 열면, 실제 내용은
// 숨겨진 `#codeVal`(class="hidden")에만 채워지고 화면에 보이는 `#showCode`(<pre>)는 항상
// 비어있던 버그였다 — legacy `service/yona.code.Browser.js`가 `#codeVal` -> Ace
// 에디터(`#showCode`)로 복사했는데, 그 JS가 어떤 템플릿에서도 로드되지 않아 연결이 끊겨 있었다.
// Ace/AJAX 트리 완전 이식은 별도 범위이고, 여기서는 "내용이 안 보이는" 핵심 버그만 고친다.
    ...
    init {
        ...
        describe("code/view.html 일반 텍스트/소스 코드 파일 내용 렌더링 (P3-63)") {
            it("바이너리도 마크다운도 아닌 파일을 열면 실제 내용이 #showCode 안에 보여야 하고, hidden 클래스 뒤에 숨어있으면 안 된다") {
                ...
                // (CodeBrowserListWrapRenderingSpec과 동일 패턴) — gitDir.exists()만 보면 오판해
                // 커밋을 건너뛴다. HEAD 파일 존재로 판단해야 안전하다.
                ...
                // 버그는 "내용이 존재하지만 hidden에 숨어 화면에 안 보임"이었다.
