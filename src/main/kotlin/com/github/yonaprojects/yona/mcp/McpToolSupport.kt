...
// yona-wiki P3-07(MCP 서버) Step3 — MCP 도구(@Tool) 메서드에서 던지는 오류. Spring AI의
// MethodToolCallback은 도구 메서드가 던진 예외를 잡아 CallToolResult(isError=true, 메시지 그대로)로
// 변환해 MCP 클라이언트에 돌려준다(Spring AI Tool Calling 공식 문서 확인) — 별도의 예외 처리
// 인프라를 새로 만들 필요가 없다.
class McpToolException(message: String) : RuntimeException(message)

// yona-wiki P3-07 Step3 — IssueRestApiController/PullRequestApiController와 동일한 얇은 어댑터
// 원칙: 기존 IssueController/PullRequestController/CommentController가 반환하는
// ResponseEntity<T>(REST API와 완전히 동일한 반환값)를 MCP 도구의 반환값으로 그대로 풀어주거나,
// 실패 상태코드를 사람이 읽을 수 있는 McpToolException으로 변환한다 — 신규 비즈니스 로직 없음.
...
// yona-wiki P3-07 Step3 — addReviewer/removeReviewer처럼 본문이 없는(ResponseEntity<Unit>) 동작용.
