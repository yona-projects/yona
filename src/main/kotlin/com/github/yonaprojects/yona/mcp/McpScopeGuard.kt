...
/**
 * MCP 도구 호출마다(호출 전에) 토큰 스코프를 검증하는 단일 지점.
 * 보안 요구사항("토큰 스코프가 실제로 각 도구 호출 전에 검증되는지, 특히
 ...
 * require()를 호출한다).
 *
 * 이중 인증(OAuth JWT / Fine-grained PAT) 각각 다른 방식으로 스코프를 담고 있어 분기한다:
 ...
 *   이 클래스가 그 토큰을 꺼내 기존 ApiTokenAuthorizer(이미 검증해둔 순수 판정 로직, 여기서
...
class McpScopeGuard {
    ...
    private fun requireJwtScope(authentication: JwtAuthenticationToken, group: ApiTokenScopeGroup, permission: ApiTokenPermission) {
        // Spring Authorization Server가 기본으로 발급하는 액세스 토큰의 "scope" 클레임은 공백으로
        // 구분된 문자열이 아니라 JSON 배열이다(예: {"scope":["issues:read","issues:write"]}).
        // getClaimAsString()은 배열 클레임에 대해 null을 반환해(타입 불일치) 이 메서드가 스코프를
        // 하나도 못 읽고 항상 거부하는 회귀가 McpToolsEndToEndSpec(실제 MCP 클라이언트로 발급받은
        // 진짜 토큰을 쓰는 유일한 테스트)에서만 드러났다 — getClaimAsStringList()로 배열/문자열 두
        // 표현을 모두 다루도록 고친다.
