...
class BranchApiController(
    ...
    fun setAsDefault(
        ...
        // yona BranchApp @IsOnlyGitAvailable(클래스 레벨, IsOnlyGitAvailableAction.isGit())
        // 대응. Git이 아닌 프로젝트(SVN 등)에 브랜치 조작을 요청하면 400으로 명확히
        // 거부해야 한다 — 이전에는 이 가드가 없어 SVN 프로젝트에도 그대로 진행되다 실질적으로
        // 아무 일도 하지 않으면서(no-op) 302 리다이렉트로 성공 신호를 돌려주는 오탐이 있었다.
        // yona error/badrequest.scala.html 대응 — 프로젝트는 이미 찾았으므로
    ...
    fun deleteBranch(
        ...
        // 거부해야 한다 — 이전에는 이 가드가 없어 SVN 프로젝트에도 그대로 진행되다 실질적으로
        // 아무 일도 하지 않으면서(no-op) 302 리다이렉트로 성공 신호를 돌려주는 오탐이 있었다.
        // yona error/badrequest.scala.html 대응.
        if (project.vcs?.uppercase() != "GIT") {
            model.addAttribute("project", project)
            model.addAttribute("menuType", "code")
            return "error/badrequest"
        ...
        val loginUser = authentication?.let { userRepository.findByLoginId(it.name).orElse(null) }
        // yona BranchApp @IsAllowed(Operation.DELETE)(resourceType 기본값 PROJECT) 대응
        // — PROJECT 리소스의 DELETE는 매니저/조직관리자 전용이다(isGlobalResourceAllowed()의
