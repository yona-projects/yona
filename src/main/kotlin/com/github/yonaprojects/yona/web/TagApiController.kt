...
// BranchApiController.deleteBranch()와 정확히 같은 패턴의 태그 삭제 액션(웹 UI의 code/tags.html
// "삭제" 버튼이 호출). 태그는 브랜치와 달리 웹 UI에 생성 폼을 두지 않는다(브랜치도 웹 UI로 직접
// 만들 수 없다 — PullRequestServiceImpl이 내부적으로만 createBranch()를 호출한다, GitHub도 태그
// 생성은 보통 `git push --tags`/API로 하지 릴리즈 없이 독립된 "새 태그" 버튼을 web UI에 두지
// 않는다). 생성은 REST API/CLI 전용이다.
@Controller
