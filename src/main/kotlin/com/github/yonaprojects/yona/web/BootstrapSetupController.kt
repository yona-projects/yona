...
class BootstrapSetupController(
    ...
    fun setupAdmin(
        ...
        // legacy(Play/Ebean)는 application.secret을 재기록해야 해서 서버 재시작이 필요했지만
        // (welcome/restart.scala.html), 지금 구조(Spring Boot+JPA)엔 그 메커니즘 자체가 없다 —
        // 방금 만든 계정은 같은 트랜잭션에서 즉시 커밋되므로 재시작 없이 바로 로그인할 수 있다.
