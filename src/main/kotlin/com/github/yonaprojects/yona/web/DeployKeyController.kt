...
// GitHub "Settings > Deploy keys" 화면과 동일한 저장소별 Deploy Key 등록/조회/삭제 UI.
// WebhookController와 동일한 컨벤션(Operation.UPDATE = 프로젝트 매니저 이상만).
@Controller
class DeployKeyController(
    ...
    // Post/Redirect/Get 패턴을 쓴다(발급 직후 화면에 바로 렌더링하지 않음). 처음엔 ApiToken
    // 발급 화면(edit_tokens.html)처럼 같은 POST 응답에서 바로 렌더링했으나, 이 컨트롤러가
    // 렌더링하는 project/setting_deploykeys.html은 project/menu 프래그먼트가
    ...
    // 500으로 실패하는 문제를 실제로 겪었다. 리다이렉트로 새 GET 요청을 발생시키면 project를 그
    // 요청에서 새로 조회하므로 이 문제가 없다 — 발급된 HTTPS 토큰은 RedirectAttributes 플래시
    // 속성으로 다음 GET 한 번에만 노출한다.
