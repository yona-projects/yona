...
// yona-wiki P3-14 1라운드 — [[p3-02]]의 ApiTokenAuthorizer(PAT용)와 나란히 두는 OAuth2 JWT 스코프
// 판정 순수 함수. PAT은 ApiToken 엔티티(scopedProjects 등)를 직접 대조하지만, OAuth v1 토큰은
// 프로젝트 단위로 세분화되지 않으므로([[p3-07]] 완료 로그 "토큰 스코프 축" 참고 — 항상
// allRepositories=true와 동등) repo-scope 체크가 필요 없다. 그래서 ApiTokenAuthorizer의 시그니처를
// 확장하는 대신 별도 오브젝트로 둔다 — PAT 경로에 새 개념(리소스 무관 스코프 문자열 매칭)을 섞지
// 않기 위함.
