...
// JdbcOAuth2AuthorizationService(Spring 공식 구현)가 attributes/
...
// 역직렬화 시점에 InvalidTypeIdException으로 거부된다(DCR/등록/토큰 발급 자체는 통과하지만 동의
// 처리 단계에서 저장한 attributes를 나중에 다시 읽을 때 터진다). Spring이 공식적으로 제공하는
// 확장 지점(SecurityJacksonModules.getModules(ClassLoader, BasicPolymorphicTypeValidator.Builder))을
// 그대로 써서 이 클래스 하나만 화이트리스트에 추가한다 — 화이트리스트 검증 메커니즘 자체를
// 우회/비활성화하지 않는다(보안 유지).
//
// 위와 완전히 동일한 문제가 SAML2 로그인 경로에도 있었다. 관리자가
...
// SAML 로그인 후 `/oauth2/authorize` → 동의(consent) 화면까지는 정상 도달했지만(principal.name만
// 쓰는 나머지 로직은 문제 없음), "Authorize" 클릭 시 OAuth2AuthorizationConsentAuthenticationProvider가
// 저장된 OAuth2Authorization을 다시 읽어오는 시점(JpaOAuth2AuthorizationService.findByToken)에
// InvalidTypeIdException이 났다. YonaUserDetails 때와 동일한 방식(화이트리스트에 클래스
// 하나 추가)으로 수정 — 우회 없음.
