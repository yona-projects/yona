...
enum class EventType(val messageKey: String, val order: Int) {
    ...
    // PullRequestService.submitReview()가 APPROVE/REQUEST_CHANGES/COMMENT 판정을 남길 때마다
    // 기록한다. PULL_REQUEST_REVIEW_STATE_CHANGED(14번, addReviewer/removeReviewer의 리뷰어
    // 참여/취소 전용)와는 별개다 — 자기등록 여부와 실제 판정은 서로 다른 사건이다. 메시지 키
    // "notification.type.pullrequest.reviewed"는 messages*.properties에 이미 존재하던 미사용 키를
    // 그대로 재사용한다(전 로케일 파일에 이미 있음).
