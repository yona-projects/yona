...
interface WikiService {
    ...
    // oldTitle이 null이면 신규 생성, oldTitle != newTitle이면 이름변경까지 함께 반영한다.
    // message가 비어 있으면 "Create <title>"/"Update <title>"/"Rename <old> to <new>" 형태의
    // 기본 메시지를 만든다. 반환값은 새로 생성된 커밋 id.
    fun savePage(
        project: Project,
        user: User,
        oldTitle: String?,
    ...
    // 지정 리비전(커밋) vs 부모의 unified diff — PathFilter로 그 페이지 파일만 스코프.
