...
class DiagnosticService(
    ...
    // yona MailboxService.java의 Diagnostic.register(SimpleDiagnostic { checkOne() }) 대응.
    // ImapMailboxPoller는 yona.mailbox.imap.enabled=true일 때만 빈으로 등록되므로 null 허용.
    private val imapMailboxPoller: ImapMailboxPoller? = null,
    private val mailSender: JavaMailSender? = null,
    // yona Config.java의 "application.port가 설정됐는데 application.hostname이 없으면 port가
    // 무시된다" 경고 대응. yona는 hostname/port 두 설정을 조합해 URL을 만들지만,
    ...
    fun checkAll(): List<String> {
        ...
        // 4. IMAP 메일 수신기 상태 점검 (yona MailboxService.java 대응)
        imapMailboxPoller?.healthCheckMessage()?.let { errors.add(it) }

        // 5. 절대경로 URL 기준값 설정 점검 (yona Config.java 대응)
        if (baseUrl.isBlank()) {
            errors.add(
