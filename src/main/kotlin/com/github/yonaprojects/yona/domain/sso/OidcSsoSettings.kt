...
/**
 * 관리자 UI(`SsoAdminController`)가 등록/조회/수정하는 범용 OIDC IdP 설정. 사이트 전체에 하나만
 * 존재하는 싱글턴 행(id 고정값 SINGLETON_ID)으로 관리한다 — LDAP+OIDC+SAML2를 동시에 여러 개
 * 등록하는 멀티테넌트 요구사항은 범위 밖으로 두고 복잡한 우선순위/폴백 정책까지는 설계하지 않는다.
 *
