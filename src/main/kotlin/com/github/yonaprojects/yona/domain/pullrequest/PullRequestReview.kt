...
// yona-wiki P3-15(PR 승인/변경요청 워크플로) — GitHub의 Approve/Request changes/Comment에 대응하는,
// "PR 전체에 대한 판정" 개념. CommentThread(코드 라인별 리뷰 스레드, ThreadState는 OPEN/CLOSED뿐)와는
// 완전히 별개의 신규 엔티티다 — 코디네이터 설계 결정 1번: 기존 구조가 이미 "스레드"(코드 리뷰
...
//
// 재판정 시 이력 보존(설계 결정 2번, GitHub와 동일) — 같은 리뷰어가 여러 번 판정을 남기면 매번 새
...
//
// stale 승인 자동 무효화(설계 결정 4번)는 구현하지 않는다 — 새 커밋이 push돼도 기존 승인 로우는
