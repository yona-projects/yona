...
// Issue/Posting은 (project_id, number) UNIQUE 제약이 있는데 PullRequest만
// 없었다. 그래서 채번 경쟁(동시 PR 생성)이 나면 issue처럼 500으로라도 막히지 않고 서로 다른 PR이
// 같은 번호를 갖는 조용한 데이터 손상이 났다(동시 요청 다건이 전부 같은 번호로 성공하는 것을
// 실제로 확인함). 이 제약이 그 경쟁을 "명확한 제약 위반 실패"로 바꿔주고,
// PullRequestController.createPullRequest()가 그 실패를 잡아 전체를 재시도해 각 요청이
// 서로 다른 고유 번호를 받도록 한다.
@Entity
@Table(
    name = "pull_request",
...
class PullRequest(
    ...
    // 라벨은 domain/project/Label(프로젝트 레벨 토픽 태그, project_label 조인테이블,
    // 프로젝트 검색/분류용으로 도입)과 domain/issue/IssueLabel(카테고리에 종속된 진짜
