...
interface AttachmentRepository : JpaRepository<Attachment, Long> {
    // yona AttachmentCache.get(containerType, containerId) 대응. 첨부파일 단건이
    ...
    // yona Attachment.cleanupTemporaryUploadFilesWithSchedule()의 .ge("createdDate", ...)
    // 대응 — "오래된" 파일이 아니라 "최근(threshold 이후)" 파일을 대상으로 하는 게 맞는지
    // 의심스러운 비교 방향이지만, 사용자 지시로 레거시를 그대로 포팅한다.
    fun findByContainerTypeAndCreatedDateGreaterThanEqual(containerType: ResourceType, createdDate: Instant): List<Attachment>

    // yona Attachment.findBy(Attachment) 대응 — 동일 컨테이너에 동일 이름·내용으로
