...
data class InboundEmailMessage(
    ...
    // text/plain 파트가 있으면 그 내용, text/plain이 아예 없고 text/html뿐이면 그 원본 HTML.
    val textBody: String,
    // 위 textBody가 원본 HTML인지 여부. true면 cid 치환 후 그대로(마크다운 처리기가 렌더링 시점에
    // 이미 OWASP sanitizer를 거치므로 안전) 저장한다.
    val isHtml: Boolean = false,
    val attachments: List<InboundAttachment> = emptyList()
)

// yona CreationViaEmail.saveAttachment()/saveAttachments() 대응.
