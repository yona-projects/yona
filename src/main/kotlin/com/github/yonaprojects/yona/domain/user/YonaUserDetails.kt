...
// yona-wiki P3-07(MCP 서버) Step6(회귀 수정, 2026-09-06) — OAuth2 동의(consent) 처리 과정에서
...
// 직렬화하는데, 생성자 파라미터명(passwordVal/authoritiesVal)과 이름이 달라 왕복 역직렬화가 실패했다
// (실제 authorization code flow 통합테스트로 발견 — ValueInstantiationException: passwordVal이 null).
...
class YonaUserDetails @JsonCreator constructor(
    ...
    // yona-wiki P3-07 Step6 — isEnabled()/isAccountNonLocked()는 state를 다른 의미로(단방향) 파생시킬
