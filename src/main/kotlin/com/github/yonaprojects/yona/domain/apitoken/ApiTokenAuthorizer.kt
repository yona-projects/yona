...
// "ApiToken + 대상 ResourceType + 요청 저장소 + 필요 권한" 조합의 허용 여부를 판정하는 순수 함수.
// Spring 빈으로 만들지 않은 이유는 DB/HTTP 등 어떤 인프라도 필요 없는 순수 판정 로직이라(단위
// 테스트만으로 충분히 검증 가능) 굳이 서비스로 감싸 DI 오버헤드를 지울 이유가 없어서다 —
// 필요해지면(예: 컨트롤러에서 재사용) 언제든 얇은 @Service 래퍼를 얹을 수 있다.
object ApiTokenAuthorizer {
    ...
    // 그룹/권한 매트릭스는 아예 보지 않고 만료 여부 + repo scope 일치 여부만 확인한다.
    fun isAuthorized(
        token: ApiToken,
