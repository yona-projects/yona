...
// ResourceIndicatorTokenCustomizer가 발급 시점에 스탬핑한 aud 클레임이 이 리소스 서버 자신을
// 가리키는지 확인한다 — 다른 리소스 서버용으로 발급된 토큰을 그대로 받아주는 "토큰 패스스루"를
// 막는다(계획 문서 보안 검증 항목). yona-wiki P3-14 — MCP 전용이 아니라 리소스 서버 일반(현재
// MCP/API 두 종류)에 재사용하도록 이미 리소스 무관하게 설계돼 있었다(requiredAudience만 다르게
// 주입).
