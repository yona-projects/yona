...
/**
 * `config/oauth2/YonaOAuth2User.kt`와 동일한 취지. `getName()`이 IdP의 원본 sub 클레임이 아니라
 * 로컬 User.loginId를 반환해야, Git/SVN 인증 필터나 `userRepository.findByLoginId(authentication.name)`
 * 같은 기존 관례가 그대로 동작한다.
 */
class YonaOidcUser(
    val user: User,
