...
// GitAuthorizationFilter(HTTPS)가 쓰던 "로그인 사용자가 이 프로젝트에 저장소 접근이 가능한가"라는
// 순수 판정 로직을 SshAuthServiceImpl(SSH)도 재사용할 수 있도록 GitAccessPolicy로 추출했었다.
// SvnAuthorizationFilter가 이 로직을 그대로 복붙해두고 있던 것을 발견해(GitAuthorizationFilter/
// SvnAuthorizationFilter의 requiresAuth/isMember/isGuestUser가 완전히 동일한 코드였음) config/git
// 밖으로 옮기고 이름을 VCS 중립적으로 바꿔 세 곳(Git HTTP/SVN HTTP/SSH) 모두가 공유하도록
// 정리했다(순수 리팩터링, 동작 변화 없음). 이후 `config/hg/HgAuthorizationFilter`도 이 클래스를
// 그대로 재사용해 추가됐다 — hg wire protocol 고유의 read/write 판정(cmd=unbundle /
// api/.../rw/...)만 그 필터에 남고 나머지 판정 로직은 동일하다.
@Component
class RepoAccessPolicy(
    ...
    // yona AccessControl READ 규칙 대응: PROTECTED도 PUBLIC과 동일하게 인증 없이 clone
    ...
    // yona AccessControl.isAllowedIfGroupMember() 대응. 직접 멤버가 아니어도 프로젝트가
