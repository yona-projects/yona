 ...
 *
 * `/api/v1/projects/{owner}/{project}/{resource}` 네임스페이스로 들어오는 요청은
 * ApiTokenRepository의 스코프 토큰으로만 인증하고, 스코프 밖이면 403으로 거부한다. 그 외 기존
 * URL(레거시 `/api/projects/{id}/...` 등)은 기존 UserRepository.findByToken 기반 "매칭되면 전권
 * 부여" 동작을 그대로 유지한다 — 기존 전권 토큰 사용자 마이그레이션 전략이 아직 미정이라
 * (docs/yona-wiki/plans/p3-02-cli-and-rest-api.md 참고) 레거시 경로를 강제로 끊지 않는다.
 *
 * 리소스 세그먼트가 없는 두 경로(개별 프로젝트 조회 `/api/v1/projects/{owner}/{project}`, 목록
 * `/api/v1/projects/{owner}`)도 스코프 토큰으로 인증한다:
 * - 개별 조회는 "metadata" 스코프(그룹/권한 매트릭스 없이 repo scope만 확인, GitHub의 "Metadata:
 *   Read-only" 자동 부여와 동일한 개념)로 취급한다.
 * - 목록은 "인증됨/아님"만으로 부족해(어떤 프로젝트가 보이는지는 컨트롤러가 결정해야 함) 403을
 *   내지 않고 SecurityContext에 신원만 세팅한 뒤, 인증에 쓰인 ApiToken 객체를 request
 *   attribute(SCOPED_API_TOKEN_ATTRIBUTE)로 다운스트림 컨트롤러에 넘긴다.
 *
 * owner 세그먼트조차 없는 계정 수준 URL들:
 * - `POST /api/v1/projects`(프로젝트 생성) — 신규 프로젝트를 만들 권한은 특정 프로젝트에 종속될
 *   수 없는 "계정 수준" 판정이라, project는 null로 판정하는 대신 반드시 `allRepositories=true`인
 *   토큰만 허용한다(GitHub Fine-grained PAT의 "All repositories" 토큰만 새 저장소 생성이 가능한
 *   것과 동일한 논리).
 * - `GET /api/v1/user/issues/status`("내 이슈 현황") — ISSUES 그룹으로 취급하되 project는
 *   null(여러 프로젝트에 걸친 집계라 단일 project로 좁힐 수 없음).
 * - `GET /site/export`(사이트 전체 백업) — SITE_SETTING → ADMINISTRATION 그룹으로 취급하고,
 ...
 *   없으므로(extractToken이 null 반환) 영향 없다 — PAT 헤더를 실제로 들고 오는 CLI 요청에만
 *   적용된다.
 *
 * `GET /api/v1/user/status`(`gh status` 대응, 담당 이슈+담당 PR+리뷰요청 PR을 한 번에 내려줌)는
 * ISSUES 스코프 하나만으로는 부족하다(PR 데이터도 같이 내려주므로). `AccountLevelTarget.resourceType`을
 ...
 * `authenticateScopedList()`와 동일하게 신원만 확인하고 인증에 쓰인 ApiToken을 request
 * attribute로 넘겨, 실제 스코프 판정은
...
class ApiTokenAuthenticationFilter(
    ...
    // 특정 프로젝트에 종속되지 않는 "계정 수준" 판정(신규 프로젝트 생성, 로그인 사용자 전체 이슈
    // 현황, 사이트 전체 백업 등). project를 아예 두지 않고 판정하므로
    ...
    private fun authenticateAccountLevel(
        ...
        val requiredPermission = requiredPermissionFor(request.method)
        // `/api/v1/user/status`처럼 한 URL이 여러 스코프 그룹(ISSUES + PULL_REQUESTS)에 걸친
        // 데이터를 한 번에 내려줄 수 있어 resourceTypes를 리스트로 두고 AND로 판정한다(전부
        // 통과해야 허용) — 다른 계정 수준 URL은 전부 원소 1개짜리 리스트다.
    ...
    // owner 전용 목록 경로(`/api/v1/projects/{owner}`)는 특정 프로젝트 하나가 아니라 "owner 밑
    // 전체"에 대한 요청이라 여기서 403을 내지 않는다. 어떤 프로젝트가 보이는지는 컨트롤러가
    // request attribute로 넘겨받은 ApiToken을 보고 직접 필터링한다. `/mcp` 요청도 동일한 이유(이
    // 필터 하나로는 어떤 스코프가 필요한지 알 수 없음)로 이 메서드를 재사용한다 — McpScopeGuard가
    // request attribute의 ApiToken을 꺼내 실제 판정을 한다.
    ...
    companion object {
        ...
        // 리소스 세그먼트가 없는 개별 프로젝트 조회(`/api/v1/projects/{owner}/{project}`, 정확히
        // 2세그먼트). scopedApiPattern은 3세그먼트를 요구하므로 이 URL과 겹치지 않는다. 이 URL은
        // URL을 바꾸지 않고도 "metadata" 스코프로 취급한다(resourceSegmentToResourceType["metadata"] 참고).
        private val individualProjectPattern = Pattern.compile("^/api/v1/projects/([^/]+)/([^/]+)/?$")

        // owner만 있는 목록 경로(`/api/v1/projects/{owner}`, 정확히 1세그먼트). 위 두 패턴과
        // 세그먼트 수가 달라 겹치지 않는다.
        private val ownerOnlyPattern = Pattern.compile("^/api/v1/projects/([^/]+)/?$")

        // Spring AI MCP 서버 스타터의 Streamable HTTP 단일 엔드포인트(`/mcp`, 기본값). 스코프
        // 판정은 이 필터가 하지 않는다(위 클래스 KDoc 참고).
        private val mcpPattern = Pattern.compile("^/mcp(/.*)?$")

        // owner 세그먼트조차 없는 프로젝트 "생성" 자체(`POST /api/v1/projects`). 위 세 패턴 모두
        // 최소 1개의 세그먼트(owner)를 요구하므로 겹치지 않는다.
        private val projectCreatePattern = Pattern.compile("^/api/v1/projects/?$")

        // 로그인 사용자 전체를 대상으로 하는 신규 API(`/api/v1/user/issues/status` 등).
        ...
        // 사이트 전체 관리 API(`/site/**`, `/sites/**`). 실제 사이트 관리자 권한 여부는
        // SecurityConfig의 hasAnyRole("ADMIN","SITE_ADMIN")이 별도로 검사하므로, 이 필터는 PAT
        ...
        // resourceSegmentToResourceType 매핑을 재사용한다.
        ...
        // permissions는 ProjectPermissionRestApiController가 위임하는
        // ProjectMemberController.listMembers()가 프로젝트 매니저 권한을 요구하므로 "settings"와
        // 동일하게 ResourceType.PROJECT_SETTING(ADMINISTRATION 그룹)으로 맞춘다.
        private val resourceSegmentToResourceType: Map<String, ResourceType?> = mapOf(
            ...
            // git 태그는 코드/브랜치와 같은 저장소 콘텐츠라 "code"와 동일하게
            // ResourceType.CODE(CODE 그룹)로 매핑한다 — TagRestApiController가 실제로 요구하는
            // AccessControl 권한과는 별개로, 이 필터는 PAT 스코프 그룹만 판정한다.
        ...
        // `/api/v1/**`의 URL → ResourceType/필요 권한 추론 로직은 원래 PAT 인증
        // (authenticateScoped/authenticateAccountLevel)에서만 쓰였다. OAuth2 JWT로 같은
        // 네임스페이스를 호출하는 제3자 앱에도 동일한 스코프 판정을 적용하기 위해, 이 순수 URL
        // 파싱 부분만 별도 공개 함수로 노출한다. PAT 쪽 기존 로직(ApiToken 엔티티 조회, repo-scope
