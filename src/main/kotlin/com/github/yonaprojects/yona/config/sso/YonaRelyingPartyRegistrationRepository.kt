...
/**
 * 관리자 UI(`SsoAdminController`, GitHub의 조직 SAML SSO 설정 화면과 동일한 필드 구성)로 등록한
 * IdP 메타데이터를 매 요청마다 읽어 `RelyingPartyRegistration`을 조립한다. Spring Boot 4.1.1은
 * `spring.security.saml2.relyingparty.*` 프로퍼티 기반 자동구성 모듈을 아직 제공하지 않아
 * (spring-boot-security-saml2 같은 모듈이 없음) 어차피 수동 빈 등록이 필요했다 — DB/설정 파일
 * 어느 쪽에서 읽어오든 이 저장소가 유일한 통합 지점이 된다.
 */
@Component
