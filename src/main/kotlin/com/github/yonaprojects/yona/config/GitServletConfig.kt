...
class GitServletConfig(
    ...
    // 위키 저장소를 git clone/push로 처음 접근할 때(웹 UI로 페이지를 한 번도 저장하지 않은
    // 상태) 지연 초기화하는 데 쓴다 — WikiServiceImpl이 첫 페이지 저장 시 만드는 것과 동일한
    // 기본 브랜치로 맞춘다. 기존 포지셔널 생성자 호출부를 깨지 않도록 맨 뒤에 둔다.
