...
| 19 | [i] | `common/markdown.scala.html` | `site/layout.html :: markdown(project)` (인라인) | 확인 완료 — 완전 일치(TASK-0227 조사 중 대조 완료, 코드 변경 없음). **2026-09-11 호출부 완전성 재감사**: legacy 실제 호출부 18곳 중 `code/svnDiff.scala.html`이 문서에 언급되지 않았고, 실제로 yona `code/svnDiff.html`엔 마크다운 도움말 트리거 자체가 없음(에디터 자체가 없으므로 당연히 동반 누락) — **P3-56**(커밋 댓글 폼 전체 축소 구현)에 이미 포함되는 격차라 별도 티켓 없이 P3-56에서 함께 처리 |
| 20 | [x] | `common/editor.scala.html` | `site/layout.html :: markdownEditor(name,value,editorMode)` (인라인) | **재검증 완료(TASK-0254)**. 이전 기록("백엔드에 via email 기능 자체가 없어 저가치 판단")은 stale — `domain/mail/{IncomingMailProcessingService,ImapMailboxPoller,...}`로 via-email 백엔드는 이미 완비돼 있음(`docs/parity/index.md` P0-02). 그러나 legacy 전체(`git/edit,create`, `board/edit,create`, `milestone/create,edit`, `issue/edit,create`, `common/{commentForm,commentUpdateForm,reviewForm}`, `partial_comment_form_on_thread.scala.html`(최상위, PR 코드리뷰 스레드 답글 폼) 12개 호출부)를 grep한 결과 `viaEmail=true`로 호출하는 곳이 legacy 자체에 단 한 곳도 없음(전부 기본값 false) — 파라미터가 legacy에서도 항상 관측 불가능한 값이라는 결론 자체는 맞았음. `data-via-email="false"`는 이미 yona에 정확히 반영돼 있었고, 대신 실제 렌더링 버그를 하나 발견해 수정: 미리보기 영역의 `class="markdown-preview markdown-wrap content-body"`가 `editorMode` 파라미터를 무시하고 항상 `content-body`로 하드코딩돼 있어(`code-review-body`/`comment-body`/`commit-comment-body`로 호출하는 곳들의 CSS가 깨져있었음) `th:classappend="${editorMode}"`로 수정. **2026-09-11 호출부 완전성 재감사**: 위 12개 나열에서 `partial_comment_form_on_thread.scala.html`이 이름으로 빠져 있었으나(개수 "12개"는 원래도 포함해 정확히 셌던 것으로 확인) 별도 격차는 아님 — `pullrequest/partial_comment_form_on_thread.html`이 `site/layout :: markdownEditor('contents','','code-review-body')`를 정확히 사용 중임을 재확인 |
| 21 | [x] | `common/fileUploader.scala.html` | `site/layout.html :: scripts`(tplAttachedFile/tplDropFilesHere) + `common/uploadForm.html`(신규) | 완료(TASK-0227, TDD). tplAttachedFile/tplDropFilesHere는 이미 정확히 이식돼 있었음(확인). `common.uploadForm(...)` 호출 부분은 #22에서 처리. legacy 실제 호출부 9곳: `git/edit,create`, `issue/edit,create`, `board/edit,create`, `common/commentForm`, `milestone/edit,create` — 전부 공용 스크립트 템플릿이라 캐치롤을 캡처된 파라미터 없이 전역 include하는 구조, 개별 호출부 나열 불필요 |
| 22 | [x] | `common/uploadForm.scala.html` | `common/uploadForm.html`(신규 생성) | 완료(TASK-0227, TDD). **중대 발견**: `issue/view.html`/`board/view.html`의 기존 `#upload-drop-zone`/`input[name=file]` 마크업이 legacy 구조(`upload-wrap`/`data-resource-type`/`input[name=filePath]`)와 전혀 다른 독자 구현이었고, 어떤 정적 JS 파일도 `upload-drop-zone`/`upload-file-input` 셀렉터를 참조하지 않아(grep 확인) 사실상 동작하지 않는 죽은 마크업이었음. legacy 구조로 교체. legacy 실제 호출부 3곳(`common/fileUploader`, `common/reviewForm`, `partial_comment_form_on_thread.scala.html`) — 각각 #21/#40/#187에서 개별 추적 |
...
| 24 | [i] | `common/commentForm.scala.html` | `issue/view.html`, `board/view.html` (각 페이지에 인라인) | 확인: `<form id="comment-form" ... enctype="multipart/form-data">` + 에디터 + fileUploader 슬롯 + 제출 버튼 구조는 이미 정확히 대응됨(#22에서 enctype 누락도 함께 수정). `common.editor(...)`/`common.fileUploader(...)` 자리에 해당하는 하위 조각들의 이식 상태는 #20/#21/#22 참고. **2026-09-11 호출부 완전성 재감사**: legacy 실제 호출부는 이 2곳 외에 `git/viewChanges.scala.html`(PR Changes 탭), `code/diff.scala.html`, `code/svnDiff.scala.html` 총 5곳이었음이 뒤늦게 확인됨. `git/viewChanges` 누락은 P3-55로 이미 완료(별도 파일 `common/commentForm.html` 신규 작성). `code/diff.scala.html`은 `NonRangedCodeCommentThread` 아키텍처 기반의 `code/diff.html`이 `markdownEditor` 프래그먼트로 기능적으로 동등하게 구현돼 있어 격차 아님(재확인). `code/svnDiff.scala.html`은 대응 기능이 마크다운 에디터/파일첨부/멘션 전부 없는 완전 별개의 축소 구현으로 남아있어 **P3-56**으로 신규 등록 |
| 25 | [x] | `common/commentUpdateForm.scala.html` | `common/commentUpdateForm.html`(신규) | 완료(TASK-0243). 댓글 인라인 수정 폼 — legacy는 풀페이지 POST(`action=".../id"`)지만 yona 백엔드가 REST(`PUT .../comments/{id}`)뿐이라 fetch PUT로 AJAX 전환(필요한 아키텍처 차이, 문서화). 알림메일 체크박스는 마크업만 유지하고 실제 발송억제 로직은 `CommentRequest`/`CommentService`에 파라미터가 없어 연결 보류(별도 백엔드 확장 필요, 문서화된 보류). legacy 실제 호출부: `issue/partial_comment.scala.html`, `board/partial_comments.scala.html` 2곳 — 공용 fragment 하나로 양쪽에서 동일하게 재사용(2026-09-11 #29~31 대댓글군 재감사에서 issue/board 양쪽 모두 정상 동작 확인, 같은 계열이라 격차 없다고 판단) |
| 26 | [x] | `common/commentDeleteModal.scala.html` | `common/commentDeleteModal.html`(신규) | 완료(TASK-0243). `yobi.Comment.js` 정적 자산이 없어 동등한 삭제확인+DELETE+DOM제거 로직을 인라인 스크립트로 직접 구현(필요한 아키텍처 차이). **2026-09-11 호출부 완전성 재감사**: legacy 실제 호출부 6곳(`git/view.scala.html`(=PR 상세 `pullrequest/view.html`), `git/viewChanges.scala.html`, `issue/view.scala.html`, `code/diff.scala.html`, `code/svnDiff.scala.html`, `board/view.scala.html`) 중 `pullrequest/view.html`이 `common/commentDeleteModal :: commentDeleteModal(...)`을 정확히 쓰고 있음을 재확인(격차 아님). `code/svnDiff.scala.html`만 이 프래그먼트를 안 쓰고 `confirm()`+fetch DELETE 즉석 구현으로 남아있어 **P3-56**으로 신규 등록(commentForm 부재와 같은 티켓으로 묶음) |
...
| 28 | [i] | `common/commentAndVoterPairDisplay.scala.html` | `issue/partial_view_child.html`(인라인) | 확인 완료 — `.item-count-groups` 조합 표시 구조 완전 일치(코드 변경 없음). **2026-09-11 정정**: yona 대상 컬럼이 잘못 기재돼 있었음 — legacy 실제(유일한) 호출부는 `issue/list.scala.html`이 아니라 `issue/partial_view_child.scala.html`(하위이슈 1건 렌더링, #134)이며, 대응 yona 파일은 `issue/partial_view_child.html`. 실제 코드(#134에서 TASK-0260으로 이미 이식)에 `item-count-groups`/`comments-count`/`vote-count` 구조가 정확히 존재함을 재확인(격차 아님, 문서 오기 정정) |
| 29 | [x] | `common/child_commentForm.scala.html` | `common/child_commentForm.html`(신규) | 완료(TASK-0243). `CommentController`가 이미 `parentCommentId`를 받으므로 POST AJAX로 그대로 연결. legacy 실제 호출부: `issue/partial_comment.scala.html`, `board/partial_comments.scala.html`(`common/childComments.scala.html` 경유) 2곳 — 2026-09-11 재감사로 board 쪽도 issue와 동등하게 정상 동작 확인(`board/view.html`이 `common/childCommentsAnchorDiv`/`common/childComments` fragment를 issue/view.html과 동일 시그니처로 호출, `BoardViewController.kt`도 `IssueViewController`와 동일한 parentComment 필터링/그룹핑 로직 보유) |
| 30 | [x] | `common/childComments.scala.html` | `common/childComments.html`(신규) | 완료(TASK-0243). 대댓글 목록+인라인 답글폼. `IssueViewController`/`BoardViewController`가 부모별 children 맵을 새로 계산해 넘겨준다(대댓글은 최상위 타임라인에서 제외 — 이전에는 필터가 없어 대댓글이 최상위 댓글과 중복 노출되는 버그였음, 이번에 함께 수정). legacy 실제 호출부: `issue/partial_comment.scala.html`, `board/partial_comments.scala.html` 2곳, 2026-09-11 재감사로 둘 다 확인(격차 아님, #29 참고) |
| 31 | [x] | `common/childCommentsAnchorDiv.scala.html` | `common/childCommentsAnchorDiv.html`(신규) | 완료(TASK-0243). yona는 대댓글 작성 후 페이지 새로고침 방식(REST 전환에 따른 필요한 단순화)이라 실질적 삽입 로직은 없지만 DOM 앵커(`#comment-N`)는 legacy와 동일하게 유지. legacy 실제 호출부: `issue/partial_comment.scala.html`, `board/partial_comments.scala.html` 2곳, 2026-09-11 재감사로 둘 다 확인(격차 아님, #29 참고) |
...
| 34 | [i] | `common/showSubtasksCheckbox.scala.html` | `issue/list.html`(인라인) | 확인 완료 — `#two-column-mode-checkbox`/`#toggle-show-subtasks` 구조 완전 일치(코드 변경 없음). **2026-09-11 호출부 완전성 재감사**: legacy 실제 호출부는 `issue/list.scala.html` 외에 `issue/my_partial_search.scala.html`, `issue/partial_list_wrap.scala.html`(→모두 `issue/list.html`/`issue/my_partial_search.html`에 인라인 흡수), `user/view.scala.html`도 있음 — 전부 대응 yona 화면에 정상 존재함을 재확인(격차 아님) |
| 35 | [x] | `common/tasklistBar.scala.html` | `issue/view.html`, `board/view.html`(인라인, 신규 추가) | 완료(TASK-0229, TDD). **발견**: `yona.Tasklist.js`/`gfm-task-list.js` 정적 자산은 이미 존재했지만 `.tasklist` 셸 마크업과 스크립트 로드가 두 페이지 모두에 전혀 없어 죽어있던 기능이었음. legacy와 동일 위치(본문 markdown-wrap 바로 앞)에 셸 추가 + `yona.Tasklist.js` 로드 추가. **2026-09-11 호출부 완전성 재감사**: legacy 실제 호출부는 이 2곳(본문) 외에 `issue/partial_comment.scala.html`, `board/partial_comments.scala.html`(댓글 본문)도 있음 — 댓글에는 `.tasklist` 셸과 `data-allowed-update` 속성이 둘 다 없어 진행률 바가 안 보이고 수정 권한 없는 사용자의 체크박스도 비활성화되지 않음을 확인 — **P3-60**으로 신규 등록 |
| 36 | [i] | `common/twoColumnModeCheckboxArea.scala.html` | `issue/list.html` 등(인라인) | 확인 완료 — `#two-column-mode-checkbox`/`#two-column-mode` 구조 일치(코드 변경 없음). **2026-09-11 호출부 완전성 재감사**: legacy 실제 호출부는 7곳(`git/partial_search.scala.html`(PR 검색), `issue/my_partial_search.scala.html`, `issue/partial_list_wrap.scala.html`, `organization/group_board_list.scala.html`, `organization/group_issue_search_partial.scala.html`, `board/list.scala.html`, `user/view.scala.html`). 이 중 6곳(`pullrequest/partial_search.html`, `issue/my_partial_search.html`, `issue/list.html`, `organization/boardList.html`, `organization/issueSearch_partial.html`, `user/view.html`)은 전부 정상 이식 확인, **`board/list.html` 1곳만 마크업 자체가 통째로 빠져있고 `yona.twoColumnMode.js` 스크립트만 죽은 채로 로드됨을 발견 — P3-59로 신규 등록** |
...
| 38 | [x] | `common/commitMsg.scala.html` | `common/commitMsg.html`(신규 fragment) | 완료(TASK-0243). `common/commitMsg.html` fragment 신규 작성(short span/a + 멀티라인일 때 moreBtn + hidden pre.desc). legacy 실사용처는 `code/diff.scala.html`(forceExpand=true)와 `code/history.scala.html`(short+moreBtn) 2곳뿐임을 확인(view/svnDiff는 이 fragment를 쓰지 않고 별도 인라인 span) — 두 곳 모두 fragment 재사용으로 교체. **2026-09-11 정정**: 위 "2곳뿐"은 부정확한 서술이었음 — 실제 legacy 호출부는 4곳(`code/diff`, `code/history` 외에 `git/partial_merge_result.scala.html`, `git/partial_pull_request_event.scala.html`도 `common.commitMsg(...)` 호출). 다만 이 2곳은 각각 #178(`git/partial_merge_result` — yona `pullrequest/partial_merge_result.html`이 `common/commitMsg :: commitMsg(...)`를 정확히 재사용, 완료)/#179(`git/partial_pull_request_event` — yona `pullrequest/partial_pull_request_event.html`은 PR 이벤트 스키마 제약으로 커밋별 목록 대신 메시지+링크만 표시하는 문서화된 단순화)에서 이미 개별적으로 정확히 다뤄지고 있어 실질적 감사 공백은 아님(격차 아님, 문구 정정만) |
...
| 40 | [x] | `common/reviewForm.scala.html` | `common/reviewForm.html`(신규) | 완료(TASK-0251, 그룹11). `site/layout::markdownEditor` + `common/uploadForm`으로 정확히 이식. legacy 실제 호출부: `git/viewChanges.scala.html`, `code/diff.scala.html` 2곳뿐(2026-09-11 재확인, 문서 기재와 일치) — 단, P3-55에서 `git/viewChanges` 쪽은 이 파일 자체가 삭제되고 `common/commentForm.html`(범위 없는 일반 댓글)+라인별 인라인폼으로 재구성됐으므로 최신 상태는 `docs/parity/tickets/p3-55.md` 참고 |
...
| 122 | [x] | `issue/partial_massupdate.scala.html` | `issue/list.html`, `milestone/view.html` 둘 다에 인라인 | 완료(TASK-0238). 일괄 수정 폼(상태/담당자/마일스톤/라벨 추가·제거) 구조는 대체로 일치하나, **마일스톤 드롭다운이 `project.menuSetting.milestone`(마일스톤 메뉴 활성화 여부) 게이트 없이 마일스톤 데이터만 있으면 노출**되고 있었음 — `project.isMilestoneEnabled` 조건 추가로 복구. **2026-09-11 호출부 완전성 재감사**: legacy `milestone/view.scala.html:92` `@issue.partial_massupdate(project, new SearchCondition())` 호출이 문서에 누락돼 있었으나, yona `milestone/view.html:81`에 `th:replace="~{issue/partial_massupdate :: massupdate(${project})}"`가 이미 정확히 배선돼 있음을 확인(격차 아님, 문서 보완만) |
| 123 | [i] | `issue/partial_select_label.scala.html` | `issue/list.html`, `board/view.html` 둘 다에 인라인 | 확인 완료(TASK-0238). select2 기반 카테고리별 라벨 다중선택 구조는 일치하나, legacy는 라벨 dt 안에 `isManagerOf(project)`일 때만 보이는 `[편집]` 인라인 링크를 두는 반면 yona는 별도 `.labels-wrap` 아이콘 버튼(#116에서 권한 게이트 복구함)으로 분리 배치 — 기능은 동등, DOM 구조만 소폭 차이(수용 가능한 수준으로 판단). **2026-09-11 호출부 완전성 재감사**: legacy `board/view.scala.html`도 호출부임이 문서에 누락돼 있었으나(게시글도 이슈와 같은 `IssueLabel` 테이블을 공유해 라벨을 가질 수 있음), yona `board/view.html:176-184`가 `isAllowedUpdate` 분기로 `issue/partial_select_label`/`issue/partial_show_selected_label`을 동일 조건으로 정확히 호출 중임을 확인(격차 아님, 문서 보완만) |
| 124 | [i] | `issue/partial_show_selected_label.scala.html` | `issue/partial_show_selected_label.html`(프래그먼트) | 확인 완료(TASK-0238). `dl`/`dt`/라벨 앵커 구조 일치, 수정 불필요. **2026-09-11**: `board/view.scala.html` 호출부도 #123과 함께 정상 이식 확인(격차 아님) |
...
| # | 상태 | legacy 경로 | yona 대상 경로 | 비고 |
|---|---|---|---|---|
| 143 | [x] | `board/list.scala.html` | `board/list.html` | 완료(TASK-0240, TDD). #144 조사에서 발견된 2건 수정. **2026-09-11 호출부 완전성 재감사**: legacy 45행 `@common.twoColumnModeCheckboxArea(false)` 대응 체크박스 마크업이 없어 `yona.twoColumnMode.js`(로드는 됨)가 조작할 대상이 없는 죽은 include로 남아있음을 발견 — **P3-59**로 신규 등록 |
...
| 161 | [x] | `code/svnDiff.scala.html` | `code/svnDiff.html` | 완료(TASK-0243). GNB/프로젝트헤더/메뉴, site/layout::scripts 누락 복구, 브랜치 드롭다운(#39) 추가. **2026-08-23 추가 완료**: 감시(watch) 버튼 URL 배선(`sWatchUrl`/`sUnwatchUrl`이 빈 문자열이었음)과 "목록" 링크 복구 — 그룹10 진행 로그 참고. **2026-09-11 호출부 완전성 재감사**: 이 감사 범위가 구조(GNB/헤더/메뉴/스크립트)에 한정돼 있었고 댓글 영역 자체는 검증 대상이 아니었음을 재확인 — 댓글 폼/삭제모달/권한체크/첨부파일/멘션/마크다운렌더링이 legacy `common.commentForm`/`common.commentDeleteModal`(#24/#26) 대신 완전히 별개의 축소 즉석 구현으로 남아있어 **P3-56**으로 신규 등록 |
...
| 186 | [x] | `partial_comment_thread.scala.html`(최상위) | `pullrequest/partial_comment_thread.html` | 완료(TASK-0243). 코드리뷰 스레드 렌더러(comment-avatar/media-body/meta-info + data-range-* 속성). legacy 실제 호출부: `partial_diff_comment_on_line.scala.html`(#190, 라인별 스레드 삽입 지점) 1곳뿐 — #188~191 그룹(partial_diff/partial_diff_line/partial_diff_comment_on_line/partial_filediff)과 함께 이미 조합 검증됨(2026-09-11 문서 보완만, 격차 아님) |
| 187 | [x] | `partial_comment_form_on_thread.scala.html`(최상위) | `pullrequest/partial_comment_form_on_thread.html` | 완료(TASK-0243). 실제 폼 제출(`ReviewViewController.newPullRequestComment`)이 이미 legacy와 동일한 풀페이지 POST 방식이라 AJAX 전환 없이 그대로 재사용 가능했음. legacy 실제 호출부: `code/partial_nonrange_codecomment_thread.scala.html`(#166), `partial_comment_thread.scala.html`(#186) 2곳 — 2026-09-11 재감사로 `site/layout::markdownEditor('contents','','code-review-body')`가 정확히 쓰이고 있음을 재확인(격차 아님, #20 참고) |
...
| 196 | [x] | `organization/header.scala.html` | `organization/header.html` | 완료(TASK-0250) — 신규 작성(project/header.html과 동일 패턴). 로고/브레드크럼/게스트 가입요청 드롭다운. **2026-09-11 호출부 완전성 재감사**: legacy 실제 호출부는 조직 화면들(`organization/view`, `group_pullrequest_list`, `group_board_list`, `group_issue_list`) 외에 `error/forbidden_organization.scala.html`과 `search/result.scala.html`(조직 스코프 검색)도 있음. `error/forbidden_organization.html`은 fragment를 그대로 재사용 중임을 확인(격차 아님). `search/list.html`(조직 스코프 검색결과)은 이 fragment를 전혀 안 쓰고 즉석 `.org-header` 블록으로 대체돼 있어 **P3-58**로 신규 등록 |
| 197 | [x] | `organization/menu.scala.html` | `organization/menu.html` | 완료(TASK-0250) — 신규 작성(project/menu.html과 동일 패턴). 홈/이슈/게시판/PR 탭 + 관리자 설정 톱니. **2026-09-11 호출부 완전성 재감사**: #196과 동일 — `error/forbidden_organization.html`은 정상(격차 아님), `search/list.html`(조직 스코프)은 미사용 — **P3-58**로 등록 |
...
| 236 | [x] | `help/keymap.scala.html` | `help/keymap.html` | 완료(TASK-0249, TDD). 파라미터(`section`,`project`)를 못 받는 `th:replace` 전체 include 방식이었던 것을 `th:fragment="keymap(section, project)"`로 교체(board/list.html, board/view.html 두 호출부도 함께 수정). **2026-09-11 호출부 완전성 재감사**: legacy 실제 호출부는 이 2곳 외에 `issue/view.scala.html`, `issue/partial_list_wrap.scala.html`도 있음 — 둘 다 문서에 기재되지 않았고 실제로도 미이식 상태였음(issue/view.html엔 keymap 자체가 없고, issue/list.html은 파라미터화 이전의 하드코딩 복제본이 그대로 남아있음) — **P3-57**로 신규 등록 |
