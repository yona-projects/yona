...
| P3-49 | [ ] | 이슈 댓글 화면 파일 업로드(드래그드롭/클릭/붙여넣기)가 두 가지 별개 원인으로 전부 죽어있음 (2026-09-10, P3-46 8-1단계 검증 중 사용자 질문에 답하려 Playwright로 실제 재현하다 발견, 기록만 해두고 착수는 보류) | 원인1: `issue/view.html`이 `<script>` 태그 안에 `<th:block th:if=...>`를 중첩해 써서 Thymeleaf가 처리 못 하고 그대로 출력 → 브라우저가 `SyntaxError: Unexpected token '<'`로 그 스크립트 블록 전체 파싱 실패 → `$yobi.loadModule("issue.View", ...)`가 아예 안 불려 watch/unwatch·업로더 연결 등 전부 죽음(Play→Spring 리라이트 때 생긴 포팅 버그, legacy v1.6엔 없음 — `@if(){}`는 스크립트 안에서도 정상 처리됨). 원인2: legacy의 "textarea에 드롭/붙여넣기 즉시 `POST /files`+마크다운 링크 자동삽입" 기능(`yona.CommentAttachmentsUpdate.js`)이 파일은 포팅됐지만 `<script src>`로 로드하는 곳이 하나도 없음(이번 세션에 반복 발견된 "소스는 있는데 미로드" 패턴의 4번째 사례). 부가로 `elements.uploader = $("#upload")`도 `uploadForm(...)` 호출이 전부 `formId=null`이라 항상 빈 선택자. 같은 `<script>` 안 `<th:...>` 중첩 패턴 전수조사 결과 다른 곳엔 없음(`site/layout.html`의 `text/x-jquery-tmpl` 블록 2곳 `th:text="#{...}"` 미처리는 문법은 안 깨지고 i18n만 항상 한국어 고정되는 경미한 별건) | [[tickets/p3-49]] |
