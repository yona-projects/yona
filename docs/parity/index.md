...
| P3-43 | [ ] | yona 자체(로컬) 로그인에 2FA(WebAuthn 우선 + TOTP 폴백) 선택 인증 옵션 추가 (2026-09-10 사용자 제안) | legacy v1.6엔 2FA 자체가 없어(완전 신규 기능) 동치성 이식 대상 아님. 계정별 선택적 2단계 인증 — WebAuthn/TOTP 둘 다 등록 가능, 로그인 시 WebAuthn 우선 시도, 실패/거부 시 곧장 TOTP로 넘기지 않고 "다시 시도"/"다른 방법 사용(TOTP)" 두 선택지 노출(구글 로그인 방식, 사용자 확정). 설계 결정 확정: 신규 테이블 3개 분리(`user_totp_credential`/`user_webauthn_credential`/`user_backup_code`, `ssh_key` 패턴과 일관), WebAuthn은 Spring Security 7.1.1 내장 지원(서드파티 불필요) 재사용, TOTP는 `dev.samstevens.totp`, 백업코드 8개·재발급 시 전량 무효화, 관리자 강제 비활성화 경로 필수 — 착수는 보류, 백로그 등록만 지시받음. 상세는 티켓 참고. | [[tickets/p3-43]] |
