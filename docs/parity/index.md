...
| P3-70 | [~] | 잔여 jQuery 의존성 전면 vanilla JS 전환(자체 작성 JS 33개 + 템플릿 인라인 79개 + 코어 라이브러리) (2026-09-14, 사용자 요청 — 기존 "jQuery 의존성 점진적 축소" 캠페인의 연속) | 10라운드 계획(`docs/parity/tickets/p3-70.md` 참고) — `static/javascripts/lib/` 벤더 파일은 절대 미수정(jQuery 기반이든 vanilla든), 우리 코드의 호출부만 네이티브/vanilla로 교체. 라운드마다 TDD+코디네이터 검증+Playwright 실측 후 커밋, 사용자 승인으로 중단 없이 연속 진행 중 | [[tickets/p3-70]] |
