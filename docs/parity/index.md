...
| P3-43 | [ ] | yona 자체(로컬) 로그인에 2FA(TOTP 또는 WebAuthn 택일) 선택 인증 옵션 추가 (2026-09-10 사용자 제안) | legacy v1.6엔 2FA 자체가 없어(완전 신규 기능) 동치성 이식 대상 아님. 계정별 선택적 2단계 인증 — 방식은 TOTP 또는 WebAuthn(보안 키/생체인증) 중 계정마다 선택(GitHub 계정 보안 설정 관례) — 착수는 보류, 백로그 등록만 지시받음. 범위 확정 필요 사항(시크릿/credential 저장 방식, TOTP·WebAuthn 라이브러리, Spring Security 필터 체인 통합 방식, 백업 코드 정책)은 티켓 참고. | [[tickets/p3-43]] |
