...
| P3-46 | [x] | 클라이언트 위젯 라이브러리 현대화 — Select2/Pikaday/Jcrop/atjs/ViewerJS/marked.js/highlight.js가 오래되거나 유지보수가 느림 (2026-09-10 사용자 제안, [[tickets/p3-45]] 논의 중 파생) | **완료(2026-09-14 최종 갱신)**. 1~7번(Select2→Tom Select, Pikaday→Flatpickr, Jcrop→Cropper.js, atjs→Tribute.js, ViewerJS→순정 Viewer.js 업그레이드, marked.js v18/highlight.js v11 버전업)은 전부 완료. 8번(마크다운 에디터)은 애초 계획했던 EasyMDE 재스킨을 넘어, CodeMirror5의 구조적 결함(백스페이스/Delete 시 Tribute.js 컨텍스트 손실)으로 인해 **CodeMirror6 기반 자체 제작 Web Component `<yona-markdown-editor>`**(별도 저장소 `yona-projects/components`, TypeScript+esbuild 빌드 후 vendoring)로 전면 재작성 — 툴바 9개 커맨드, 서버 렌더링 미리보기, `@codemirror/autocomplete` 기반 `@`/`:`/`#` 멘션까지 EasyMDE+Tribute.js를 완전히 대체. 16개+ 화면 Playwright 전수 검증(307/309, 나머지 2건은 테스트 타이밍 오탐) 완료. **2026-09-14**: vendoring 산출물이 컴포넌트 저장소보다 3개 커밋 뒤처져 있어 그사이 올라간 **저장형 XSS 수정(멘션 후보 렌더링, `escapeHtml` 누락)**과 폼 reset 데이터손실 수정이 미반영 상태였음을 발견·즉시 재빌드/vendoring으로 해소(상세: [[tickets/p3-46]] "CM6-8단계" 절) | [[tickets/p3-46]] |
...
| P3-56 | [x] | SVN 커밋 상세 화면(`code/svnDiff.html`)의 댓글 기능이 legacy 대비 크게 축소된 즉석 구현으로 남아있음 — 마크다운 에디터/파일첨부/멘션/권한기반삭제/삭제확인모달/마크다운렌더링 전부 없음 (2026-09-11, TEMPLATE_BACKLOG.md 호출부 완전성 감사에서 발견 — `common/commentForm.scala.html`(#24)/`common/commentDeleteModal.scala.html`(#26)의 실제 legacy 호출부에 `code/svnDiff.scala.html`이 있었는데 문서에 전혀 언급되지 않아 검증망을 빠져나간 사례, P3-55와 동일 패턴의 재발) | **완료(커밋 `ddeed748b`)**. 백엔드(`ReviewViewController`/`ReviewApiController`/`CodeReviewServiceImpl`)는 이미 legacy와 동일한 댓글 CRUD+권한체크를 갖추고 있어 순수 템플릿 레이어 문제였음을 확인 — `code/svnDiff.html` 댓글 섹션을 `code/diff.html`과 동일하게 `markdownEditor`+`uploadForm`+`commentDeleteModal`(권한기반 삭제)로 통일. `SvnCommitCommentTemplateRenderingSpec`(7/7 GREEN)+Playwright 실측 검증 완료 | [[tickets/p3-56]] |
| P3-57 | [x] | 이슈 상세/이슈 목록 화면에 키보드 단축키 안내(`help/keymap`) 도움말이 빠져 있거나(issue/view.html) 리팩터 이전 방식(issue/list.html에 하드코딩 복제)으로 남아있음 (2026-09-11, TEMPLATE_BACKLOG.md #236 호출부 완전성 감사 — 실제 legacy 호출부 4곳 중 문서가 board 2곳만 "함께 수정"했다고 기록, issue 쪽 2곳이 감사망을 빠져나감) | board/view.html·board/list.html은 TASK-0249에서 파라미터화된 `help/keymap :: keymap(section, project)` fragment로 정상 전환됐지만 issue 쪽은 대상에서 누락됨. **완료** — issue/view.html·issue/list.html에도 동일 fragment 배선, 매니저 전용 항목("설정"/"코드 주고받기") 노출 확인. `IssueKeymapHelpTemplateRenderingSpec`(6/6 GREEN)+Playwright 실측 검증 완료 | [[tickets/p3-57]] |
| P3-58 | [x] | 조직(organization) 범위 검색 결과 화면(`search/list.html`)에 조직 헤더/내비게이션 메뉴(`organization/header`, `organization/menu`)가 빠져 있음 (2026-09-11, TEMPLATE_BACKLOG.md #196/#197 호출부 완전성 감사 — 실제 legacy 호출부에 `search/result.scala.html`이 있었는데 문서에 전혀 기재되지 않아 발견) | `error/forbidden_organization.html`은 동일 fragment를 이미 정상 재사용 중(격차 아님, 대조군). `search/list.html`만 즉석 `.org-header` 블록으로 대체돼 있어 조직 탐색 메뉴/로고/브레드크럼/가입요청 드롭다운이 전부 사라짐. **완료** — `search/list.html`에 `organization/header`+`organization/menu` fragment 배선. `OrganizationSearchHeaderTemplateRenderingSpec`(3/3 GREEN)+Playwright 실측 검증 완료 | [[tickets/p3-58]] |
| P3-59 | [x] | 게시판 목록(`board/list.html`)에 "2단 모드" 체크박스 마크업이 없어 `yona.twoColumnMode.js`가 로드는 되지만 조작할 대상이 없는 죽은 스크립트 include로 남아있음 (2026-09-11, TEMPLATE_BACKLOG.md #36 호출부 완전성 감사 — `common/twoColumnModeCheckboxArea.scala.html`의 실제 legacy 호출부 7곳 중 문서가 "확인 완료"로 뭉뚱그린 나머지를 개별 검증하다 발견) | 나머지 6개 호출부(issue/list, issue/my_partial_search, user/view, organization/boardList, organization/issueSearch_partial, pullrequest/partial_search)는 전부 정상 이식 확인됨 — board/list.html 한 곳만 예외. 저위험(스타일/토글 UI). **완료** — `board/list.html`에 2단 모드 체크박스 마크업 추가. `BoardListTwoColumnModeTemplateRenderingSpec`(2/2 GREEN)+Playwright 실측 검증 완료(issue/list의 "자식이슈 펼쳐보기"와 id 중복 발견은 범위 밖으로 별도 등록 권장) | [[tickets/p3-59]] |
| P3-60 | [x] | 이슈/게시글 댓글 본문의 GFM 체크리스트(tasklist)에 진행률 바/상호작용 셸(`.tasklist`)이 없음 — 본문(issue/board 상단)에만 있고 댓글에는 없음, `data-allowed-update` 속성도 댓글 쪽엔 없어 수정 권한 없는 사용자의 체크박스가 비활성화되지 않는 문제도 함께 발견 (2026-09-11, TEMPLATE_BACKLOG.md #35 호출부 완전성 감사 — `common/tasklistBar.scala.html`의 실제 legacy 호출부 4곳 중 댓글 쪽 2곳(issue/partial_comment, board/partial_comments)이 문서에 기재돼 있지 않아 발견) | `issue/view.html`/`board/view.html` 본문에는 `.tasklist` 셸+`data-allowed-update`가 있지만 댓글 루프에는 둘 다 없음(각 파일 grep 결과 `.tasklist` 매치 1건뿐). **완료** — 댓글 루프에도 `.tasklist` 셸+`data-allowed-update` 배선, 체크박스 클릭 시 PATCH가 실제로 `.../comments/{id}`로 나가 저장되고 새로고침 후에도 유지됨을 확인(진행 중 `commentUpdateForm`의 action 누락도 발견·수정). `CommentTasklistProgressBarTemplateRenderingSpec`(5/5 GREEN)+Playwright 실측 검증 완료 | [[tickets/p3-60]] |
...
| P3-63 | [x] | 코드 파일 보기 화면에서 소스 내용 자체가 렌더링되지 않음(Ace 에디터 미포팅의 부수 효과) (2026-09-12, JS 인벤토리 작업 중 `service/yobi.code.Browser.js`가 어느 템플릿에서도 로드되지 않는 것을 발견해 legacy 대조하다가 예상보다 심각한 별도 버그를 발견) | `code/view.html`의 실제 파일 내용(`#codeVal`)에는 `class="hidden"`이 붙어 있고, 화면에 보이는 `#showCode`는 완전히 빈 채로 렌더링됨 — 둘을 연결하던 `yobi.code.Browser.js`(Ace 에디터 초기화)가 SPA 트리 단순화 결정과 함께 통째로 안 로드됨. 바이너리/마크다운 파일은 정상, 일반 텍스트/소스 코드 파일만 해당. **완료** — Ace 에디터 초기화 스크립트를 실제 로드되도록 배선(문법강조·줄번호 거터 포함). 회귀 스펙 다수 GREEN+Playwright 실측(Java 문법강조 토큰 렌더링, 바이너리/마크다운 경로 회귀 없음) 검증 완료 | [[tickets/p3-63]] |
| P3-64 | [x] | "맨 위로 스크롤" 버튼(jQuery elevator)이 이슈/게시글 상세 화면에 없음 (2026-09-12, JS 인벤토리 작업 중 `lib/elevator`가 어느 템플릿에서도 로드되지 않는 것을 발견) | legacy v1.6.1에는 파일 자체가 없었고(2018-06-11 추가) 최신 legacy에서는 board/issue 상세 화면에서 실사용 중. yona엔 vendoring만 돼 있고 로드하는 곳이 없음. 순수 UX 편의 기능이라 저위험·저우선순위. **완료** — board/issue 상세 화면에 스크립트 로드+`$.elevator(...)` 호출 배선(legacy 대조 결과 issue/view는 `tooltips:true`, board/view는 `glass:true`로 서로 다름을 확인해 정확히 재현). `ScrollToTopElevatorTemplateRenderingSpec`(GREEN)+Playwright 실측 검증 완료 | [[tickets/p3-64]] |
| P3-65 | [x] | 첨부된 동영상이 Video.js 스킨 없이 브라우저 기본 컨트롤로만 재생됨 (2026-09-12, JS 인벤토리 작업 중 `lib/videojs`가 어느 템플릿에서도 로드되지 않는 것을 발견) | legacy v1.6.1에는 파일 자체가 없었고(2020-06-14 추가) 최신 legacy에서는 이슈 상세 화면에서 실사용 중. 조사 결과 yona의 sanitizer/`yobi.Attachments.js`는 이미 Video.js 호환 마크업(`class="video-js"`+`<source>`)을 정상 생성하고 있어 실제 재생은 브라우저 기본 컨트롤로 문제없이 동작함 — 순수 시각적 스킨 격차뿐, 기능 손실 없음. **완료** — issue/view.html에 Video.js CSS/JS 로드(별도 초기화 스크립트 불필요, 기존 마크업을 Video.js가 자동 스캔). `AttachmentVideoJsSkinTemplateRenderingSpec`(GREEN)+Playwright 실측 검증 완료 | [[tickets/p3-65]] |
| P3-66 | [x] | 이슈 공유자(Sharer) 입력이 검색형 위젯이 아니라 순수 텍스트 입력으로 퇴화 (2026-09-12, JS 인벤토리 작업 중 `service/yona.issue.Sharer.js`가 어느 템플릿에서도 로드되지 않는 죽은 코드임을 재확인 — 파일 자체에 P3-46 #5 시점의 "범위 밖 발견" 코멘트가 이미 있었음) | legacy엔 hidden input+TomSelect류 위젯으로 실제 동작 중. yona는 백엔드 API(`IssueShareController`의 findSharer/sharableUsers/share 3종)와 프론트 스크립트(`yona.issue.Sharer.js`, TomSelect 이식 완료) 둘 다 이미 준비돼 있고 `issue/view.html`에 `<script src>`+초기화 호출만 빠져있음 — 4개 티켓 중 착수 난이도 최저. **완료** — issue/view.html에 `<script src>`+`yonaIssueSharerModule(...)` 초기화 호출 배선. `IssueSharerWidgetWiringTemplateRenderingSpec`(GREEN)+Playwright 실측(검색/추가/제거) 검증 완료 | [[tickets/p3-66]] |
| P3-67 | [x] | 웹훅 등록 폼(`project/setting_webhook.html`)에 Payload URL 필수 입력 클라이언트측 검증이 없음 — `service/yona.project.Webhook.js`가 legacy와 동일하게 존재하지만 로드되지 않음 (2026-09-14, 사용자 요청으로 자체 작성 JS 76개 전수(직접 script 태그+minify-js.sh 번들 재료+`$yona.loadModule` 동적 로드 3경로 전부 고려) 감사 중 발견) | 서버측(`WebhookController.kt:82`, `payloadUrl.isBlank()`)이 최종 방어선이라 데이터 무결성 문제는 아님(빈 값으로 웹훅이 실제 생성되지는 않음) — 순수 UX 격차, 우선순위 낮음. 같은 감사에서 함께 확인한 `yona.Interval.js`/`yona.Sha1.js`는 [[tickets/p3-52\|P3-52]]가 이미 "재구현됨/원래 죽은 코드"로 결론 낸 것과 일치, `yona.organization.Member.js`/`yona.project.Member.js`는 REST API 직결로 설계상 대체(템플릿에 문서화됨), `yobi.WatcherList.js`/`yobi.ui.Mergely.js`/`yobi.CodeTemplate.js`는 legacy에도 로드된 적 없는 완전한 죽은 코드 — 전부 격차 아님. **완료** — `<script src>`+`$yona.loadModule("project.Webhook",...)` 배선(`project/delete.html`/`transfer.html`과 동일 관례). 실측 결과 payloadUrl에 이미 있던 HTML5 `required`가 커스텀 검증보다 먼저 개입해 제출을 막고 있어 실질 격차는 "안내 채널 차이(네이티브 vs 커스텀)"였음을 확인 — 핵심 동작(즉시 차단)은 동치. `TemplateEquivalenceSpec`(87/87)+`WebhookControllerSpec`(19/19)+`WebhookRestApiControllerSpec`(1/1) GREEN+Playwright 실측 검증 완료 | [[tickets/p3-67]] |
| P3-68 | [x] | `ResponseStatusException`의 reason 메시지가 에러 응답에 노출되지 않음(웹훅 등록에서 발견) — P3-67 구현 중 클라이언트 검증을 우회해 직접 POST하면 서버가 HTTP 400은 반환하지만 메시지가 응답 바디에 없음 (2026-09-14, P3-67 TDD 에이전트가 범위 밖으로 보고, 사용자 지시로 별도 등록) | 완료(2026-09-14). 원인은 Spring Boot 설정 `spring.web.error.include-message` 기본값 `never`(Boot 4에서 `server.error.*`가 아니라 `spring.web.error.*`로 이동돼 있었음). 80곳 표본 전수 확인 결과 노출돼도 안전한 일반 문구뿐이라 전역으로 `always` 설정 + 커스텀 에러 템플릿(400/401/403/404.html)에 `message` 폴백 추가. RANDOM_PORT 통합테스트로 RED→GREEN 확인 | [[tickets/p3-68]] |
| P3-69 | [x] | PR 뷰 화면의 머지 가능 여부(mergeable/conflict) 상태가 실시간 자동 갱신되지 않음 — legacy `service/yobi.git.View.js`(10초 폴링)에 대응하는 파일이 현재 없음 (2026-09-14, 사용자 요청으로 JS 파일을 legacy와 stem 단위 전수 재대조하는 감사 중 발견 — 이전 감사는 '로드 여부'만 확인해 '대응 파일 자체가 없는 것'을 놓쳤음) | 완료(2026-09-14). 신규 GET `/{owner}/{projectName}/pull/{number}/state`(`partial_state`+Accept 버튼 합성 프래그먼트 반환) + 신규 `service/yona.pullrequest.View.js`(10초 폴링, skip-if-unchanged, CLOSED/MERGED/isMerging 시 중단). Accept 버튼을 named fragment(`partial_info::acceptButton`)로 분리해 `#state`와 함께 갱신. Playwright로 실제 git 충돌을 mid-poll에 만들어 새로고침 없이 배너+버튼이 자동 전환됨을 실측(+CLOSED 상태에서 폴링 0건도 확인) | [[tickets/p3-69]] |
| P3-70 | [x] | 잔여 jQuery 의존성 전면 vanilla JS 전환(자체 작성 JS 33개 + 템플릿 인라인 79개 + 코어 라이브러리) 완료 (2026-09-14~15, 사용자 요청 — 기존 "jQuery 의존성 점진적 축소" 캠페인의 연속) | 최초 10라운드 계획에서 시작해 jQuery 코어 제거 과정에서 발견된 실의존들(Bootstrap2 dropdown/button/alert DATA-API, 로그아웃 fetch 전환, EasyMDE data() 브릿지, bootstrap-switch.js 대체 위젯)까지 총 12라운드로 완료(`docs/parity/tickets/p3-70.md` 참고). `static/javascripts/lib/`·`bootstrap/js/` 벤더 파일은 전혀 수정하지 않고 참조만 제거, 라운드마다 TDD+코디네이터 검증+Playwright 실측 후 커밋. 메인 사이트 번들에서 `jquery-3.3.1`/`bootstrap.js` 로드 자체가 완전히 빠졌다(격리된 legacy 화면 4개는 각자의 jQuery 1.9 사본을 그대로 유지, 캠페인 범위 밖) | [[tickets/p3-70]] |
| P3-71 | [x] | 프로젝트 가져오기 화면의 "저장소 인증정보" 체크박스가 반전 동작함(체크하면 닫히고 언체크하면 열림) (2026-09-14, P3-70 라운드5 작업 중 발견 — 전환이 만든 새 버그가 아니라 원본 jQuery 코드에도 있었을 구조적 결함을 소스 레벨(jquery-ui.custom.js:11151)로 증명해 1:1 재현) | 같은 `change` 이벤트에 이중으로 걸린 두 핸들러(템플릿 단순 핸들러 + `project.New.js`의 `.toggle("slide")`)를 하나로 통합 — `_toggleSlide(el, bShow)`가 체크박스 checked 상태를 직접 받아 열지/닫을지 판단하도록 바꾸고 템플릿의 단순 핸들러는 제거. 검증 중 별개의 사전 존재 버그(`*{owner == org.name}` 선택 표현식이 th:each 지역변수를 폼 필드로 오인해 500 에러)도 함께 발견·수정. Playwright + 관련 스펙 3개 GREEN | [[tickets/p3-71]] |
| P3-72 | [ ] | `PullRequestCommentFormWiringSpec`이 오래된 전제(review-form 완전 제거)로 실패 중 (2026-09-14, P3-70 라운드6 작업 중 발견 — 코디네이터가 라운드5 커밋 시점으로 되돌려 재현해 라운드6과 무관함을 직접 확인) | `pullrequest/view.html`이 `canReviewComment` 조건으로 여전히 `common/reviewForm`을 렌더링하는데(그 화면 자체 히스토리 주석에 "4단계 복원" 기록 있음), 테스트는 review-form이 완전히 제거됐다고 가정하던 시절 그대로 남아 `shouldNotContain "id=\"review-form\""`로 실패. 의도된 최종 설계인지 재확인 후 테스트 갱신 또는 마크업 정리 필요 | [[tickets/p3-72]] |
| P3-73 | [x] | 코드리뷰 댓글 팝업(`review-form`)에서 타이핑이 불가능함 — CodeMirror 뷰가 destroy된 채 복구되지 않음 (2026-09-15, P3-71/P3-72 재현 스크린샷을 보여주다 사용자가 "글자를 입력할 수 있는 칸이 안 보인다"고 지적해 발견) | `_placeReviewForm()`의 무조건적 `appendChild`가 `<yona-markdown-editor>`를 disconnect/reconnect시켜 CodeMirror 뷰를 destroy하는데 `connectedCallback`의 중복초기화 방지 가드 때문에 복구되지 않음(P3-55 1차 시도에서 한 번 고쳤다가 4단계 복원 때 유실된 것과 동일 버그). 불필요한 재배치 방지 가드 + 실제 이동 후 `cloneNode` 기반 remount로 수정, Playwright로 실제 타이핑/제출/반복사용 확인 | [[tickets/p3-73]] |
