...
| P3-72 | [x] | `PullRequestCommentFormWiringSpec`이 오래된 전제(review-form 완전 제거)로 실패 중 (2026-09-14, P3-70 라운드6 작업 중 발견 — 코디네이터가 라운드5 커밋 시점으로 되돌려 재현해 라운드6과 무관함을 직접 확인) | review-form이 여전히 렌더링되는 것은 "code.Diff.js 복원 4단계"(legacy CodeCommentBox 재도입, P3-73으로 마저 다듬어짐)로 의도된 최종 설계임을 재확인 - 테스트의 4개 `shouldNotContain` 단언을 `shouldContain`으로 뒤집어 현재 마크업 계약에 맞게 갱신. review-form(라인/범위 댓글)과 common/commentForm(범위 없는 일반 댓글)은 서로 다른 두 기능이 의도적으로 공존 | [[tickets/p3-72]] |
