...
| P3-42 | [ ] | GitHub/Forgejo 수준의 프로젝트 위키 기능 추가 (2026-09-09 사용자 제안, 백로그 등록만 하고 착수 보류 지시) | legacy v1.6엔 위키 자체가 없어(완전 신규 기능) 동치성 이식 대상 아님. `ResourceType.WIKI_PAGE`/`ApiTokenScopeGroup.WIKI`는 스코프 축 자리만 파여있고 실 기능(컨트롤러/템플릿/도메인) 0줄임을 확인. 목표 수준은 사용자 지정으로 Forgejo(페이지 CRUD/`_Sidebar`·`_Footer`·`Home` 특수 페이지/페이지별 히스토리·diff/커밋 메시지 커스터마이징/git clone 접근/검색) — 상세는 티켓 참고. **미착수** | [[tickets/p3-42]] |
