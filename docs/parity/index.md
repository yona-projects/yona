...
| P3-55 | [ ] | PR "Changes" 탭 코드리뷰 댓글 UX가 두 갈래(범위없는 `#review-form`/라인별 인라인삽입 폼)로 쪼개져 있고, `#review-form`은 트리거가 없어 죽어있고 라인별 쪽은 `<yona-markdown-editor>`가 아예 안 붙어있음 (2026-09-11, P3-46 CM6-7단계 16개 화면 전면 회귀 중 발견 — 원인은 마크다운 에디터와 무관한 오래된 뷰 배선 문제라 별도 분리) | `#review-form` 안의 CM6 에디터 자체(렌더링/타이핑/툴바 9개/멘션 3트리거/미리보기)는 강제로 표시해 확인한 결과 전부 정상 동작 — 순수 UI 배선 문제. 3가지 설계 방향(죽은 폼 제거/살리기/라인폼 서버렌더링 전환) 중 선택은 코디네이터 판단 필요, 착수 보류 | [[tickets/p3-55]] |
