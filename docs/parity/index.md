...
| P3-48 | [ ] | (긴급) CSRF 재활성화 이후 raw `$.ajax` PUT/DELETE/PATCH 호출이 CSRF 토큰 없이 403으로 거부될 가능성 (2026-09-10, P3-47 검증 중 발견) | board/edit 저장을 Playwright로 끝까지 재현하니 `PUT /api/projects/{id}/posts/{id}`가 403 — `$.ajax`가 CSRF 헤더를 안 실음. 전역 CSRF 자동 주입은 `.ajaxForm()` 플러그인 경로에만 있고 raw `$.ajax`엔 없음. PUT/DELETE/PATCH를 쓰는 파일 13개 중 12개가 잠재적 영향권(CSRF 언급 1개뿐) — 전수 재현·전역 `$.ajaxSetup` 도입 필요 | [[tickets/p3-48]] |
...
| P3-50 | [ ] | 댓글 수정 폼에서 기존 첨부파일 목록이 안 보이고 관리도 불가능함 (2026-09-11, P3-49 재조사 중 발견) | `common/commentUpdateForm.html`이 legacy 전용 마크업(`.file-upload__input`/`.temporaryUploadFiles`/`.attachment-files`, `yona.CommentAttachmentsUpdate.js`가 기대하는 체계) 대신 새 글 작성용 범용 `common/uploadForm` 프래그먼트를 그대로 재사용 — 이미 첨부된 파일을 조회/렌더링하는 로직 자체가 없어 첨부파일이 있는 댓글을 수정하면 그 파일들이 전부 안 보이고 삭제도 불가능(데이터 접근 불능, 단순 UX 문제 아님). 백엔드 조회 자체는 이미 가능(`AttachmentRepository.findByContainerTypeAndContainerId`) — 화면 연결만 안 돼 있음. 착수 전 백로그 등록만 | [[tickets/p3-50]] |
| P3-51 | [ ] | `spring.messages.fallback-to-system-locale`(Spring Boot 기본값 true)로 인해 배포 환경의 JVM 기본 로케일에 따라 i18n 메시지가 요청 로케일과 무관하게 뒤바뀔 수 있음 (2026-09-11, Twirl/Thymeleaf 렌더링 감사 중 발견) | `messages_en.properties`처럼 요청 로케일 전용 파일이 없으면 root(`messages.properties`)보다 먼저 서버 JVM 시스템 로케일 파일을 시도하는 Java ResourceBundle 기본 동작 때문에, 이 샌드박스(JVM 기본 ko_KR)에서는 `Locale.ENGLISH`를 명시해도 한국어 메시지가 나온다 — 배포 환경마다 달라지는 비결정적 버그. `fallback-to-system-locale: false`로 끄면 legacy(`application.langs`의 결정적 폴백)와 동등해지지만, 이 한 줄만으로 전체 테스트 스위트에서 53건(14개 스펙 파일)이 새로 실패함을 확인 — 그동안 "로케일 미지정 요청은 한국어로 보인다"는 이 버그의 부작용에 암묵적으로 의존해 작성된 테스트가 많았다. 이번 세션 범위 밖이라 수정은 되돌리고 발견만 기록 | [[tickets/p3-51]] |
| P3-52 | [ ] | legacy가 로드하던 일부 스크립트가 포팅본에 로드되지 않아 생긴 기능 격차 묶음 (2026-09-11, Twirl/Thymeleaf 렌더링 감사 카테고리 C 전수 스캔) | 242개 legacy 뷰의 `<script src>` 목록과 포팅본을 전수 대조 — 대부분은 이미 다른 방식으로 재구현됐거나 원래 죽은 코드였음을 확인. 실제 격차로 남는 것: (1) `yona.ReceiverList.js` 미로드로 댓글 작성 중 알림 대상 실시간 미리보기 없음, (2) `yona.detectChange.js` 미로드로 이슈 화면 변경 감지 폴링 없음, (3) `yobi.CommentForm.js` 미로드로 댓글 작성 중 이탈 경고·localStorage 초안 자동저장 없음, (4) **PR 코드 리뷰 diff 화면에 새 인라인 댓글 작성 UI가 있는지 스캔 단계에서 확정 못함 — 가장 먼저 재확인 필요**(실재하면 영향 큼) | [[tickets/p3-52]] |
