...
| P3-34 | [ ] | SVN도 Git/Hg처럼 실제 서버(`RANDOM_PORT`)+실제 CLI 바이너리로 HTTP interop 검증하는 테스트가 없음 (2026-09-09 사용자 제안) | Git/Hg는 각각 `GitSmartHttpProtocolIntegrationSpec`/`HgHttpProtocolIntegrationSpec`에서 실제 서버+실제 `git`/`hg` 바이너리로 clone/push 왕복을 확정 검증하는데, SVN 관련 스펙 2개는 전부 MockMvc 기반(실제 포트 없음)이고 실제 `svn` 바이너리를 실행하는 코드가 테스트 트리에 전혀 없음(조사로 확인) | [[tickets/p3-34]] |
