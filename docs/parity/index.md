...
| P3-69 | [ ] | PR 뷰 화면의 머지 가능 여부(mergeable/conflict) 상태가 실시간 자동 갱신되지 않음 — legacy `service/yobi.git.View.js`(10초 폴링)에 대응하는 파일이 현재 없음 (2026-09-14, 사용자 요청으로 JS 파일을 legacy와 stem 단위 전수 재대조하는 감사 중 발견 — 이전 감사는 '로드 여부'만 확인해 '대응 파일 자체가 없는 것'을 놓쳤음) | 서버측 재계산(`PullRequestViewController.viewPullRequest()`의 `attemptMerge()`)은 이미 완전히 동치이고 push/머지 이벤트 기반 알림(P1-71 `notifyMergeConflictChanged`)도 있으나, "열어둔 페이지가 자동 갱신"되는 클라이언트측 폴링 자체가 없음. 사용자 확인 후 TDD로 구현 진행 결정 | [[tickets/p3-69]] |
