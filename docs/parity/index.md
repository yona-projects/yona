...
| P3-63 | [ ] | 코드 파일 보기 화면에서 소스 내용 자체가 렌더링되지 않음(Ace 에디터 미포팅의 부수 효과) (2026-09-12, JS 인벤토리 작업 중 `service/yobi.code.Browser.js`가 어느 템플릿에서도 로드되지 않는 것을 발견해 legacy 대조하다가 예상보다 심각한 별도 버그를 발견) | `code/view.html`의 실제 파일 내용(`#codeVal`)에는 `class="hidden"`이 붙어 있고, 화면에 보이는 `#showCode`는 완전히 빈 채로 렌더링됨 — 둘을 연결하던 `yobi.code.Browser.js`(Ace 에디터 초기화)가 SPA 트리 단순화 결정과 함께 통째로 안 로드됨. 바이너리/마크다운 파일은 정상, 일반 텍스트/소스 코드 파일만 해당. 착수 전 조사만 완료 | [[tickets/p3-63]] |
