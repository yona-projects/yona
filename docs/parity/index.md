...
| P3-64 | [ ] | "맨 위로 스크롤" 버튼(jQuery elevator)이 이슈/게시글 상세 화면에 없음 (2026-09-12, JS 인벤토리 작업 중 `lib/elevator`가 어느 템플릿에서도 로드되지 않는 것을 발견) | legacy v1.6.1에는 파일 자체가 없었고(2018-06-11 추가) 최신 legacy에서는 board/issue 상세 화면에서 실사용 중. yona엔 vendoring만 돼 있고 로드하는 곳이 없음. 순수 UX 편의 기능이라 저위험·저우선순위. 착수 전 조사만 완료 | [[tickets/p3-64]] |
| P3-65 | [ ] | 첨부된 동영상이 Video.js 스킨 없이 브라우저 기본 컨트롤로만 재생됨 (2026-09-12, JS 인벤토리 작업 중 `lib/videojs`가 어느 템플릿에서도 로드되지 않는 것을 발견) | legacy v1.6.1에는 파일 자체가 없었고(2020-06-14 추가) 최신 legacy에서는 이슈 상세 화면에서 실사용 중. 조사 결과 yona의 sanitizer/`yobi.Attachments.js`는 이미 Video.js 호환 마크업(`class="video-js"`+`<source>`)을 정상 생성하고 있어 실제 재생은 브라우저 기본 컨트롤로 문제없이 동작함 — 순수 시각적 스킨 격차뿐, 기능 손실 없음. 착수 전 조사만 완료 | [[tickets/p3-65]] |
| P3-66 | [ ] | 이슈 공유자(Sharer) 입력이 검색형 위젯이 아니라 순수 텍스트 입력으로 퇴화 (2026-09-12, JS 인벤토리 작업 중 `service/yona.issue.Sharer.js`가 어느 템플릿에서도 로드되지 않는 죽은 코드임을 재확인 — 파일 자체에 P3-46 #5 시점의 "범위 밖 발견" 코멘트가 이미 있었음) | legacy엔 hidden input+TomSelect류 위젯으로 실제 동작 중. yona는 백엔드 API(`IssueShareController`의 findSharer/sharableUsers/share 3종)와 프론트 스크립트(`yona.issue.Sharer.js`, TomSelect 이식 완료) 둘 다 이미 준비돼 있고 `issue/view.html`에 `<script src>`+초기화 호출만 빠져있음 — 4개 티켓 중 착수 난이도 최저. 착수 전 조사만 완료 | [[tickets/p3-66]] |
