...
| P3-68 | [ ] | `ResponseStatusException`의 reason 메시지가 에러 응답에 노출되지 않음(웹훅 등록에서 발견) — P3-67 구현 중 클라이언트 검증을 우회해 직접 POST하면 서버가 HTTP 400은 반환하지만 메시지가 응답 바디에 없음 (2026-09-14, P3-67 TDD 에이전트가 범위 밖으로 보고, 사용자 지시로 별도 등록) | 원인은 Spring Boot `server.error.include-message` 기본값 `never` — 이 컨트롤러만이 아니라 `ResponseStatusException(...)` 호출부 전체(9개 파일 80곳)에 공통 적용되는 설정. **정정**: 최초엔 "legacy 동치성 이슈"로 봤으나 재확인 결과 legacy(`Webhook.java:141-143`)는 애초에 빈 payloadUrl이면 메시지 없이 조용히 no-op이었음 — legacy 문제가 아니라 순수 서버 응답 품질 개선. 전역 설정 켤 경우 80곳 reason 문구가 전부 노출되므로 안전성 표본 확인 필요 | [[tickets/p3-68]] |
