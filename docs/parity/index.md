...
| P3-54 | [ ] | `site/postList.html`/`site/issueList.html`이 authorLoginId가 null인 게시글/이슈를 만나면 500 에러 (2026-09-11, P3-48/50/51/52 완료 후 전체 회귀 재검증 중 재확인 — Twirl/Thymeleaf 감사 baseline 실패 2건과 동일) | `userRepository.findByLoginId(...)`가 non-null 파라미터라 null을 넘기면 Spring Data 널 안전성 검사에서 즉시 예외 — `.orElse(null)`은 "못 찾았을 때"만 처리하지 "null 호출 자체"는 못 막음. 이번 4개 티켓과 무관한 기존 결함(baseline)임을 git diff로 확인. 착수 전 백로그 등록만 | [[tickets/p3-54]] |
