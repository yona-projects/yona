...
| P3-71 | [ ] | 프로젝트 가져오기 화면의 "저장소 인증정보" 체크박스가 반전 동작함(체크하면 닫히고 언체크하면 열림) (2026-09-14, P3-70 라운드5 작업 중 발견 — 전환이 만든 새 버그가 아니라 원본 jQuery 코드에도 있었을 구조적 결함을 소스 레벨(jquery-ui.custom.js:11151)로 증명해 1:1 재현) | 같은 `change` 이벤트에 이중으로 걸린 두 핸들러(템플릿 단순 핸들러 + `project.New.js`의 `.toggle("slide")`)가 실행 순서 때문에 서로의 가시성 판단을 어긋나게 만듦. "완전 동치" 원칙상 P3-70 범위에서는 고치지 않고 분리 등록 | [[tickets/p3-71]] |
