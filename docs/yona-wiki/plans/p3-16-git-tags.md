...
> **2026-09-07 ID 정정**: 작성 당시(2026-09-03) 이 계획은 자신을 "P3-10"이라 칭했으나, 이는
> `docs/PARITY_BACKLOG.md`에서 이미 2026-08-28에 "전체 의존성 최신 버전 업데이트" 항목이 선점한
> 번호와 우연히 충돌한 것이었다(별도 채번 체계가 있는 게 아니라 단순 실수) — 착수 완료 시점에
> 발견해 `docs/PARITY_BACKLOG.md`의 실제 행 번호(`P3-16`, 그 표에서 비어 있던 다음 번호)에 맞춰
> 이 파일명/frontmatter id를 `P3-10` → `P3-16`으로 정정했다. 문서 전체에서 본문 중 "P3-10"이라는
> 표기가 남아있다면 전부 이 계획(Git 태그) 자신을 가리키는 것이며 실제로는 `P3-16`으로 읽어야 한다.
...
- **프로젝트 스코프 밖 호출 방지**: `TagRestApiController`가 `findByOwnerAndNameOrPreviousPlace(owner,
  ...
  않는다.
- **경로 탈출 검증 비대칭(코디네이터 push 전 리뷰, 2026-09-07 발견·수정)**: `create()`는
  `Repository.isValidRefName()`으로 경로 탈출 문자를 걸러내는데 `delete()`는 이 검증 없이 곧바로
  `deleteTag()`를 호출하고 있었다 — JGit의 ref 삭제가 실제로 임의 파일에 영향을 줄 가능성은
  낮아 보이지만(존재 확인 후 오브젝트 파싱까지 성공해야 삭제로 이어짐), 검증을 비대칭으로 남겨둘
  이유가 없어 `delete()`에도 동일한 `isValidRefName()` 검사를 추가했다(실패 시 404 — 존재하지
  않는 태그와 동일하게 취급). MockMvc로는 URL 경로 변수에 `../`를 실어 보내는 것 자체가 URI
  정규화 때문에 신뢰성 있게 재현되지 않아, `TagRestApiControllerSpec`에 컨트롤러 메서드를 직접
  호출하는 유닛 테스트로 이 검증 로직을 고정했다.
