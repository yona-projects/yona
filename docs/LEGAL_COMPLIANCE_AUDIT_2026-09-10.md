...
| 3 | CSRF 보호가 애플리케이션 전역에서 비활성화됨 | 웹 보안 | P0 — **조사 완료, 구현은 보류(2026-09-10)** |
...
**조사 완료, 구현은 보류(2026-09-10)** — 상세 조사 결과는
[`docs/CSRF_INVESTIGATION_2026-09-10.md`](CSRF_INVESTIGATION_2026-09-10.md) 참고.
요약: (1) 왜 껐는지 — 사유 설명 없이 단일 "빅뱅" 재작성 커밋에서 도입됐고, 레거시 Play
앱도 원래 CSRF 보호가 없었다(신규 회귀 아님, 의도적 설계 근거는 없음). (2) Thymeleaf
자동 주입 여부를 실제 통합테스트 2건으로 검증 — `th:action` 폼(템플릿의 83%)은 CSRF를
켜면 추가 조치 없이 자동 보호되지만, 순수 HTML `action=` 폼(예: `site/layout.html`의
sitewide 로그인 모달)과 JS가 동적으로 만드는 폼(11개 파일, `$yobi.sendForm` 계열)은
보호 대상이 아니다. (3) `$.ajax`/`fetch` 호출 약 77곳(JS 파일 + 템플릿 인라인 스크립트
전수 조사, 파일:줄 목록은 investigation 문서 참고)이 CSRF 헤더를 별도로 실어야 한다.
(4) `/api/v1/**`·`/mcp/**`는 이미 별도 `SecurityFilterChain`이라 캐치올 체인의 CSRF
설정 변경과 무관하게 안전하지만, `/-_-api/v1/**`(레거시 전권 PAT)·레거시 `/api/**`는
별도 체인이 없어 캐치올 체인을 그대로 타므로 CSRF 예외 처리가 반드시 필요하고(안 하면
yona-cli 등 헤드리스 클라이언트가 깨짐), `/git/**`·`/svn/**`·`/hg/**`(HTTP Basic 인증)도
마찬가지로 예외 처리가 필요하다. 재활성화 자체는 (이 조사가 파악한 범위 안에서는)
"손댈 수 없을 만큼 위험하거나 범위가 크다"고 판단되지는 않았고 구체적인 설계안까지
정리했지만, **조사 도중 사용자가 이번 라운드는 조사만 하고 실제 구현은 하지 말라고
명시적으로 지시**해 코드 변경(SecurityConfig.kt/JS/템플릿)은 하지 않았다. 순수
조사/검증용 통합테스트 2건만 추가했다(investigation 문서 6절 참고).
