...
| 3 | CSRF 보호가 애플리케이션 전역에서 비활성화됨 | 웹 보안 | ~~P0~~ **완료됨(2026-09-10)** |
...
**완료됨(2026-09-10)** — 상세 조사·구현 내역은
[`docs/CSRF_INVESTIGATION_2026-09-10.md`](CSRF_INVESTIGATION_2026-09-10.md) 참고. 캐치올
보안 체인의 CSRF 보호를 `CookieCsrfTokenRepository` 기반으로 재활성화했다: `th:action` 폼은
자동 보호(템플릿의 83%), 순수 HTML `action=` 폼(sitewide 로그인 모달)은 `th:action`으로
전환, `$.ajax`/`fetch` 호출은 전역 인터셉터로 토큰을 실어 보내도록 배선, JS가 동적으로
그리는 인라인 댓글 폼(`code/compare.html`/`diff.html`)은 히든 필드 수동 주입으로 대응했다.
`/api/v1/**`·`/mcp/**`는 원래도 독립 보안 체인이라 영향 없고, `/-_-api/v1/**`·레거시
`/api/**`(PAT 인증)는 전용 체인(`LegacyApiSecurityConfig`)으로 분리해 CSRF 예외 처리를
했으며, `/git/**`·`/svn/**`·`/hg/**`(HTTP Basic 인증)도 CSRF 검증에서 제외했다. 전체 회귀
테스트 6,494건 통과. 구현 과정에서 CSRF와 무관한 별개의 기존 결함(로그아웃 성공 시 302
대신 204가 반환되던 버그)을 발견해 [[tickets/p3-44|P3-44]]로 분리·수정했다.
