...
- **PRIVATE 프로젝트 비멤버가 hg로 clone을 시도하면 (hang은 안 하지만) 파이썬 에러 메시지가
  뜬다** — 위 항목과는 별개의, 더 아래 계층(hg4j 와이어 프로토콜) 버그였다(hg4j 백로그 48번,
  2026-09-09 수정). 원인: 실제 hg SSH 클라이언트는 연결하자마자 `hello`+`between` 핸드셰이크를
  보내고 그 정상 응답을 볼 때까지 어떤 에러 라인도 안 읽는데, `SshRelayServer`가 저장소를 만들지
  않은 채(권한 검사가 저장소 접근보다 먼저이므로) 에러 한 줄만 쓰고 끊으면 이 핸드셰이크에 응답을
  못 받아 클라이언트가 무한 대기했다. `HgSshWireServer.rejectConnection()`이 핸드셰이크를 먼저
  정상 완료한 뒤 거부 사유를 내보내도록 고쳐 **hang 자체는 해소됐다.** 다만 핸드셰이크 완료 후
  real hg가 실제로 보내는 다음 명령(`listkeys namespace=bookmarks`)은 hg4j의 범용 에러 응답을
  자기 포맷(tab-분리)으로 파싱하려다 그 자리에서 실패해, 클라이언트 쪽엔 사유 텍스트가 사람이
  읽기 좋게 보이는 대신 real hg의 내부 파싱 에러가 뜬다 — **이건 의도적으로 고치지 않고 남겨둔
  것**이다. 바로 위 git 항목("clone/fetch 자체가 거부되면 메시지가 지저분하다")과 정확히 같은
  급의 트레이드오프이고, 이 문서가 git 쪽에도 이미 "실제 동작엔 영향 없는 사소한 UX 한계로
  남겨둔다"고 결정해뒀으므로 hg만 더 높은 기준으로 다듬을 이유가 없다고 판단했다(hg4j
  `llm-wiki/backlog/wire-protocol-negotiation.md` 백로그 48번 절 참고). **hang이 없다는 것만
  확인하면 정상 동작이다** — clone/fetch 거부 시 메시지가 안 예뻐도 걱정할 것 없다.
