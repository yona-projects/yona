 ...
 * yona's wire endpoints (`/hg/{owner}/{project}` -- HgController.kt, `/svn/**` -- SvnController.kt),
 * both permitAll + HTTP Basic per SecurityConfig.kt.
 *
 * A brand-new hg repo has no known remote branches yet, so the very first push must pass
 * `--new-branch` -- without it, `hg push` aborts with "push creates new remote branches: default";
 * `--new-branch` is a no-op on reruns once the branch is already known, so it is always safe to
 * pass.
 *
 ...
 * value as a positional arg launches vim non-interactively and hangs forever. `hg commit -u "..."`
 * sets the commit user directly instead.
 */
...
test('push two real hg changesets into the seeded Mercurial project', async () => {
  ...
  // can't preempt it and the process orphans forever.
...
test('commit two real svn revisions into the seeded Subversion project', async () => {
  ...
  // TEST BUG (not a yona product bug): this environment's system locale is ko_KR.UTF-8, and the
  // svn CLI localizes its own output to Korean under that locale (`svn commit` prints something
  // other than "Committed revision N." in Korean) -- the regex below is English-only, so it
  // silently failed to match, `Number(undefined)` produced NaN, and `JSON.stringify(NaN)`
  // serializes as `null` in seed.json, which requireSeed() then treats as "missing" for every
  // downstream svn-diff spec. Pin LC_ALL=C for this subprocess so its output is always in
  // parseable English regardless of the host's locale (hg is unaffected -- its `log --template`
  // output used below is raw data, not localized natural-language text, so it needed no such
  // pin).
