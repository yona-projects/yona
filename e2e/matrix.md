...
| 즐겨찾기(star): 이슈 (POST /-_-api/v1/favoriteIssues/{id}) | specs/15-misc/favorites.spec.ts | done | 이슈 상세 화면의 star 아이콘 실제 클릭 두 번(토글 on/off) 모두 실제 `locator.click()`으로 확인(과거의 `el.click()` 인페이지 디스패치 우회는 #9 수정 후 제거). **#9 수정 완료**: `yona.Usermenu.js`의 `afterUsermenuLoaded()`가 페이지 로드마다 두 번(즉시 1회 + GNB usermenu AJAX 파셜 로드 후 1회) 호출되는데, `.favorite-issue`는 이슈 상세 페이지 자신에도 서버렌더되고 AJAX 파셜(사이드바 즐겨찾기 목록) 안에도 있어 두 호출 모두에서 매치돼 클릭 리스너가 중복 바인딩됐음 — 클릭 한 번에 POST가 두 번 나가 두 번째 요청이 DB unique 제약 위반(500)을 받고, 그 에러 핸들러의 `$yona.alert()`가 연 `<yona-dialog id="yonaDialog">`(native `<dialog>.showModal()`)가 화면 전체를 덮는 `::backdrop`으로 영구히 클릭을 삼켰음(자기 host 엘리먼트의 bounding box는 0-width/화면 밖으로 보이지만, 실제로는 top layer로 승격된 dialog의 backdrop이 클릭을 가로챔). `$yona.notify()`/Toast 자체는 무관함(단독 `notify()` 호출만으로는 재현 안 됨, 라이브로 직접 확인). 수정: `yona.Usermenu.js`에 `_bindOnce(el, sType, fHandler)` 헬퍼를 추가해 `afterUsermenuLoaded()`의 모든 리스너 바인딩이 엘리먼트당 한 번만 붙도록 idempotent하게 변경. 새 회귀 테스트(`a notify() toast from the favorite-issue toggle does not block a later real click elsewhere on the page`)가 토글 직후 완전히 무관한 엘리먼트(`#sidebar-open-btn`)에 대한 실제 클릭이 정상 동작함을 확인 |
