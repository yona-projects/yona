...
| 이슈 목록의 일괄수정 위젯 — 상태(열림/닫힘) 토글 | specs/06-issue/issue-management.spec.ts | done | CLOSED→OPEN 왕복 확인. **제품 버그 발견(수정 안 함, 최종 보고 참고)**: 상태 배지 텍스트가 `#{'issue.state.' + issue.state}`로 원본(대문자) enum 이름을 그대로 메시지 키에 쓰는데 `messages*.properties`엔 소문자 키(`issue.state.closed`)만 있어, 배지 CSS 클래스(정상, 소문자 보간)는 맞지만 텍스트는 `??issue.state.CLOSED_en_US??` 플레이스홀더로 깨짐 |
| 이슈 목록의 일괄수정 위젯 — 라벨 추가/제거 | specs/06-issue/issue-management.spec.ts | fixme | **제품 버그(수정 안 함, 최종 보고 참고)**: `attachingLabelIds=<id>&issues[0].id=<id>` 페이로드로 실제 POST가 나가고 200/302도 정상 응답하지만, 이슈에 라벨이 실제로 붙지 않음(재조회해도 라벨 없음) — `state`/`assignee.id`/`milestone.id`는 전부 정상 반영되는 것과 대조적이라 `IssueMassUpdateForm.attachingLabelIds`/`detachingLabelIds`(둘 다 최상위 `List<Long>`, 다른 필드들과 달리 중첩 객체나 단순 스칼라가 아님) 바인딩 관련 문제로 추정 |
| 이슈 추천(vote)/추천취소 | specs/06-issue/issue-management.spec.ts | done(부분) | 추천은 실제 클릭으로 확인. **제품 버그 발견(수정 안 함, 최종 보고 참고)**: 추천취소 버튼의 `th:href`가 `hasVoted` 여부와 무관하게 항상 `/vote`로 고정되어 있어(`th:classappend`/툴팁만 상태에 따라 바뀜) 실제로는 `/unvote`에 도달할 방법이 UI에 없음 — 테스트는 정리용으로 `/unvote`를 직접 호출 |
| 댓글 수정/삭제 | specs/06-issue/issue-management.spec.ts | fixme | **제품 버그(수정 안 함, 최종 보고 참고)**: seed 마일스톤("E2E seed milestone (edited)", 공백·괄호 포함)이 지정된 이슈 페이지를 로드하면 브라우저 콘솔에 `Failed to execute 'querySelector' on 'Document': "E2E seed milestone (edited)" is not a valid selector`가 발생함(`page.on('pageerror')`로 실측) — 마일스톤/담당자 TomSelect 관련 스크립트(`yona.ui.TomSelect.js`/`yona.issue.Assginee.js`/`yona.issue.Sharer.js` 중 하나로 추정, 정확한 호출부는 시간 관계상 미확정) 어딘가가 옵션 텍스트를 셀렉터로 잘못 사용하는 것으로 보이며, 그 예외가 같은 인라인 스크립트 블록의 나머지 초기화(댓글 수정 버튼 `[data-toggle="comment-edit"]` 클릭 배선 포함)를 막아 버튼은 화면에 보이지만 클릭이 영원히 반응하지 않음 |
...
| 게시글 라벨 지정 (PUT /api/projects/{id}/posts/{postId}/labels) | specs/09-board/board-crud.spec.ts | done | **제품 버그 발견(수정 안 함, 최종 보고 참고)**: board/view.html이 이슈와 동일한 라벨 `<select>` 위젯을 렌더링하지만 `yona.board.View.js`에 이 select의 change를 저 PUT 라우트로 저장하는 배선이 전혀 없음(전체 정적 JS grep으로 확인) — 라벨을 선택해도 조용히 아무 일도 안 일어나는 UI 갭. 실제(고장난) 동작 그대로를 단언하는 테스트로 문서화 |
| 게시글 삭제 (DELETE /api/projects/{id}/posts/{postId}) | specs/09-board/board-crud.spec.ts | fixme | **제품 버그(수정 안 함, 최종 보고 참고)**: 사이트 전역 `yona.Common.js`의 `requestAs()`가 `[data-request-method]` 전체를 자동 배선하는데, board/view.html이 같은 삭제 버튼에 자기만의 별도 클릭 핸들러를 또 붙여 클릭 한 번에 DELETE 요청이 **두 번** 나감 — 먼저 도착한 요청이 실제 삭제(200), 나중 요청은 404를 받는데 board/view.html 자신의 핸들러가 이 404를 받으면 `window.location` 이동이 아예 안 일어나 UI가 멈춤(서버 쪽 삭제 자체는 성공 — curl로 재확인함). 경합 조건이라 `test.fixme()`로 표시 |
...
| 게스트 모드 토글 (POST /site/toggleGuestMode) | specs/13-admin/site-admin.spec.ts | done | **제품 버그 발견(수정 안 함, 최종 보고 참고)**: `SiteService.toggleGuestMode()`가 `isGuest` 불리언만 뒤집고 `state` enum은 안 건드리는데, "게스트 사용자" 탭 목록 쿼리(`UserRepository.findUsersForAdminQuery`)는 `state` 컬럼만 필터링함 — 토글해도 GUEST 탭에 절대 안 나타나고 ACTIVE 탭에서도 사라지지 않는 실제 버그. 버튼 라벨 재렌더(게스트 지정 ↔ 일반회원 전환)로 실제 토글 자체는 확인 |
| 관리자의 비밀번호 강제 초기화 (POST /site/users/{loginId}/reset-password) | specs/13-admin/site-admin.spec.ts | fixme | **제품 버그(수정 안 함, 최종 보고 참고)**: userList.html의 "비밀번호 초기화" 버튼 `data-href`가 `/{loginId}?action=resetPassword`로 잘못 빌드됨(실제 라우트는 `/site/users/{loginId}/reset-password`) — 매칭되는 컨트롤러가 없어 매번 404, 화면엔 "password change failed" 알림만 뜸 |
