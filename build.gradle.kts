...
dependencies {
	...
	// WebAuthn(보안 키/생체인증) 지원. Spring Security 7.1.1이 내부적으로 WebAuthn4J를 써서
	// 자체 내장하므로 서드파티 불필요, BOM이 버전을 관리해 별도 버전 고정 없이 추가.
	implementation("org.springframework.security:spring-security-webauthn")
	// TOTP(RFC 6238) 지원. Spring Security엔 내장 기능이 없어 별도 라이브러리 필요 —
	// QR코드 PNG 생성까지 해결하는 dev.samstevens.totp 채택. Spring Boot BOM이 버전을
	// 관리하지 않는 서드파티라 명시적으로 고정.
	...
	// WebAuthn 등록/인증 세리모니는 실제 브라우저 없이 curl로 재현 불가능해, WebAuthn4J의
	// 가상 인증기(virtual authenticator) 테스트 유틸리티로 attestation/assertion을 만들어
	// 서버 측 서명 검증까지 태우는 통합테스트에 쓴다. spring-security-webauthn이 끌어오는
