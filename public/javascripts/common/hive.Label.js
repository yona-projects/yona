...
nforge.issue.label = function() {
	var that = {
		...
		addLabelEditor : function() {
			var div = $('<div>').addClass('control-group').addClass(
					'label-editor');

			var label = $('<label id="custom-label-label">').addClass(
					'control-label').text(Messages('label.new'));

			var controls = $('<div id="custom-label">').addClass('controls');
			...
			for ( var i = 0; i < colors.length; i++) {
				var button = $('<button type="button">')
						.addClass('issue-label').css('background-color',
								colors[i]);
				controls.append(button);
			}

			var input_color = $('<input id="custom-label-color" type="text">')
					.addClass('input-small').attr('placeholder',
							Messages('label.customColor'));

			var input_category = $(
					'<input id="custom-label-category" type="text">').addClass(
					'input-small').attr('data-provider', 'typeahead').attr(
					'placeholder', Messages('label.category')).attr(
					'autocomplete', 'off');
			...
			var input_submit = $(
					'<button id="custom-label-submit" type="button">')
					.addClass('btn').text(Messages('label.add'));
		...
		updateLabels : function(callback) {
			var form = $('<form>').attr('method', 'get').attr('action',
					that.urlToLabels);
			...
			form.ajaxForm({
				success : function(responseBody, statusText, xhr) {
					...
					for ( var i = 0; i < labels.length; i++) {
						that.add_label_into_category(labels[i].id,
								labels[i].category, labels[i].name,
								labels[i].color);
					}
		...
		updateSelectedColor : function(color) {
			...
			contrasted = $hive.getContrastColor(color);
			...
			for ( var i in selectors) {
				try {
					document.styleSheets[0].addRule(selectors[i], 'color: '
							+ contrasted + ' !important');
					document.styleSheets[0].addRule(selectors[i],
							'opacity: 0.8');
				} catch (e) {
					continue;
				}
			...
			issueForm
					.submit(function() {
						...
						for ( var i = 0; i < buttons.length; i++) {
							issueForm
									.append('<input type="hidden" name="labelIds" value="'
											+ $(buttons[i]).attr('labelId')
											+ '">');
						}
		...
		removeLabel : function(id) {
			...
			if (label.siblings().size() > 0) {
				...
				source = $('#custom-label-category').typeahead().data(
						'typeahead').source;
		...
		add_del_link : function(button, id, color) {
			var delLink = $('<a>').addClass('icon-trash').addClass('del-link')
					.addClass('active-' + $hive.getContrastColor(color))
		...
		add_label_into_category : function(id, category, name, color) {
			...
			var button = $('<button type="button">' + name + '</button>')
					.addClass('btn').attr('labelId', id);
			if (that.options.editable) {
				that.add_del_link(button, id, color);
			}
			document.styleSheets[0].addRule(
					'.labels button.btn.active[labelId="' + id + '"]',
					'background-color: ' + color);
			button.addClass('active-' + $hive.getContrastColor(color));
			if (elem.length > 0) {
				elem.append(button);
			} else {
				var div = $('<div>').addClass('control-group');
				var label = $('<label class="control-label" category="'
						+ category + '">' + category + '</label>');
				var controls = $('<div class="controls" data-toggle="buttons-checkbox" category="'
						+ category + '">');
				div.append(label);
				div.append(controls);
				...
				controls.append(button);
				if (that.options.editable) {
					$('#custom-label-category').typeahead().data('typeahead').source
							.push(category);
				}
			}
					...
					.ajaxForm({
						success : function(responseBody, statusText, xhr) {
							...
							if (!(label instanceof Object)) {
								notification
										.text('Failed to add custom label - Server error.');
								return;
							}
							var label = that.add_label_into_category(label.id,
									label.category, label.name, label.color);
